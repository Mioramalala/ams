...
$(function(){
	...
	$('#enregistrer_Fermeture_Question_imc5').click(function(){	
	if((document.getElementById("commentaire_Question_imc5").value=="") || (document.getElementById("rad_Question_Oui_imc_5").checked==false && document.getElementById("rad_Question_NA_imc_5").checked==false && document.getElementById("rad_Question_Non_imc_5").checked==false)){
			...
			$('#message_fermeture_question_imc5').hide();
			...
			$.ajax({
				...
				success:function(e){
					objectif_id=e;
					if(objectif_id==0){					
						enregistrer_commentaire_f(commentaire, qcm, mission_id, 97);
					}
					else{
						$.ajax({
							type:'POST',
							url:'cycleImmo/Immoc/php/update_object_imc.php',
							data:{commentaire:commentaire, qcm:qcm, question_id:97},
							success:function(){
								$.ajax({
									type:'POST',
	...
	$('#valider_Fermeture_Question_imc5').click(function(){
		$('#message_fermeture_question_imc5').hide();
		$('#fancybox_imc').hide();
		openButtObjimc();
	});
...
<table width="500">
	...
	<tr align="center" height="50">
		<td>
			<input type="button" class="bouton" value="Oui" id="enregistrer_Fermeture_Question_imc5" />&nbsp;&nbsp;&nbsp;
			<input type="button" class="bouton" value="Non" id="valider_Fermeture_Question_imc5" />
		</td>
	</tr>
</table>
</div>