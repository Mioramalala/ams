...
$reponse = $bdd->query('SELECT OBJECTIF_COMMENTAIRE, OBJECTIF_QCM FROM tab_objectif WHERE QUESTION_ID=88 AND CYCLE_ACHAT_ID=7 AND MISSION_ID='.$mission_id);
...
<script>
$(function(){
	$('#Question_imb_19_Bouton').click(function(){
		if((document.getElementById("rad_Question_Non_imb_19").checked==true && document.getElementById("commentaire_Question_imb19").value=="") || (document.getElementById("rad_Question_Oui_imb_19").checked==false && document.getElementById("rad_Question_NA_imb_19").checked==false && document.getElementById("rad_Question_Non_imb_19").checked==false)){
				...
				success:function(e){
					...
					if(objectif_id==0){					
						...
						$.ajax({
							...
							data:{commentaire:commentaire, qcm:qcm, question_id:88, mission_id:mission_id, cycleId:7},
							success:function(e1){
								eo=""+e1+"";
								doc=eo.split(',');
								afficheReponseQuest("imb_20","imb20");
								$.ajax({
									type:'POST',
			...
			$('#Question_imb_88').fadeOut(500);
			$('#Question_imb_89').fadeIn(500);
			$('#lb_veuillez_aff_imb').hide();
		}

	});
	$('#int_imb_Question_Precedent_19').click(function(){
		$('#Question_imb_88').fadeOut(500);
		$('#Question_imb_87').fadeIn(500);
	});
	//Fermeture du question num 11
	$('#fermer_Question_imb_19').click(function(){
		$('#message_fermeture_question_imb19').show();
		$('#Question_imb_88').hide();
		//alert("Vous devez répondre à toutes les questions") ;
	});
});
</script>
<div id="int_Question_imb_19">
	<table width="500">
...
c)	qu'il ne s’écoule pas de délais anormaux entre la réception et la mise en service ?
		...
		<tr style="height:10px">
			...
			Oui<input type="radio" id="rad_Question_Oui_imb_19" name="rad_Question_imb19" <?php if($qcm=="OUI") echo 'checked'; ?> />&nbsp;&nbsp;&nbsp;
			N/A<input type="radio" id="rad_Question_NA_imb_19" name="rad_Question_imb19" <?php if($qcm=="N/A") echo 'checked'; ?> />&nbsp;&nbsp;&nbsp;
			Non <input type="radio" id="rad_Question_Non_imb_19" name="rad_Question_imb19" <?php if($qcm=="NON") echo 'checked'; ?> />
			</td>
		</tr>
		<tr style="height:10px">
		...
		<tr>
			<td align="center"><textarea id="commentaire_Question_imb19" rows="15" cols="50"><?php echo $commentaire; ?></textarea>
			</td>
		</tr>
		<tr>
			<td align="center"><input type="button" value="Precedent" class="bouton" id="int_imb_Question_Precedent_19"/>&nbsp;&nbsp;&nbsp;
				<input type="button" value="Suivant" class="bouton" id="Question_imb_19_Bouton" />
			</td>
		</tr>
	</table>
</div>
<div id="fermeture_Icone_Question_imb"><img src="../cycle_achat_image/Fermer.png" id="fermer_Question_imb_19" /></div>