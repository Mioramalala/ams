...
$(function(){
	...
	$('#enregistrer_Fermeture_Question_imb5').click(function(){	
	if((document.getElementById("rad_Question_Non_imb_5").checked==true && document.getElementById("commentaire_Question_imb5").value=="") || (document.getElementById("rad_Question_Oui_imb_5").checked==false && document.getElementById("rad_Question_NA_imb_5").checked==false && document.getElementById("rad_Question_Non_imb_5").checked==false)){
			...
			$('#message_fermeture_question_imb5').hide();
			...
			$.ajax({
				...
				success:function(e){
					objectif_id=e;
					if(objectif_id==0){					
						enregistrer_commentaire_f(commentaire, qcm, mission_id, 74);
					}
					else{
						$.ajax({
							type:'POST',
							url:'cycleImmo/immob/php/update_object_imb.php',
							data:{commentaire:commentaire, qcm:qcm, question_id:74},
							success:function(){
								$.ajax({
									type:'POST',
	...
	$('#valider_Fermeture_Question_imb5').click(function(){
		$('#message_fermeture_question_imb5').hide();
		$('#fancybox_imb').hide();
		openButtObjimb();
	});
...
<table width="500">
	...
	<tr align="center" height="50">
		<td>
			<input type="button" class="bouton" value="Oui" id="enregistrer_Fermeture_Question_imb5" />&nbsp;&nbsp;&nbsp;
			<input type="button" class="bouton" value="Non" id="valider_Fermeture_Question_imb5" />
		</td>
	</tr>
</table>
</div>