...
$(function(){
	...
	$('#enregistrer_Fermeture_Question_imb10').click(function(){
if((document.getElementById("rad_Question_Non_imb_10").checked==true && document.getElementById("commentaire_Question_imb10").value=="") || (document.getElementById("rad_Question_Oui_imb_10").checked==false && document.getElementById("rad_Question_NA_imb_10").checked==false && document.getElementById("rad_Question_Non_imb_10").checked==false)){
			...
			$('#Question_imb_').hide();
			$('#message_fermeture_question_imb10').hide();
		}
		else{
				...
				success:function(e){
					...
					if(objectif_id==0){					
						...
						$.ajax({
							...
							data:{commentaire:commentaire, qcm:qcm, question_id:79},
							success:function(){
								$.ajax({
									type:'POST',
	...
	$('#valider_Fermeture_Question_imb10').click(function(){
		$('#message_fermeture_question_imb10').hide();
		$('#fancybox_imb').hide();
		openButtObjimb();
	});
...
<table width="500">
	...
	<tr align="center" height="50">
		<td>
			<input type="button" class="bouton" value="Oui" id="enregistrer_Fermeture_Question_imb10" />&nbsp;&nbsp;&nbsp;
			<input type="button" class="bouton" value="Non" id="valider_Fermeture_Question_imb10" />
		</td>
	</tr>
</table>
</div>