...
$(function(){
	...
	$('#enregistrer_Fermeture_Question_imb4').click(function(){	
	if((document.getElementById("rad_Question_Non_imb_4").checked==true && document.getElementById("commentaire_Question_imb4").value=="") || (document.getElementById("rad_Question_Oui_imb_4").checked==false && document.getElementById("rad_Question_NA_imb_4").checked==false && document.getElementById("rad_Question_Non_imb_4").checked==false)){
			...
			$('#message_fermeture_question_imb4').hide();
			...
			$.ajax({
				...
				success:function(e){
					objectif_id=e;
					if(objectif_id==0){					
						enregistrer_commentaire_f(commentaire, qcm, mission_id, 73);
					}
					else{
						$.ajax({
							type:'POST',
							url:'cycleImmo/immob/php/update_object_imb.php',
							data:{commentaire:commentaire, qcm:qcm, question_id:73},
							success:function(){
								$.ajax({
									type:'POST',
	...
	$('#valider_Fermeture_Question_imb4').click(function(){
		$('#message_fermeture_question_imb4').hide();
		$('#fancybox_imb').hide();
		openButtObjimb();
	});
...
<table width="500">
	...
	<tr align="center" height="50">
		<td>
			<input type="button" class="bouton" value="Oui" id="enregistrer_Fermeture_Question_imb4" />&nbsp;&nbsp;&nbsp;
			<input type="button" class="bouton" value="Non" id="valider_Fermeture_Question_imb4" />
		</td>
	</tr>
</table>
</div>