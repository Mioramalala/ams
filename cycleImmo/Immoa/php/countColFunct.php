...
//check ligne : verifie si chaqhe ligne de la separation de fonction est checkée, pour immoa il ya 11 lignes à cocher
$sql = "SELECT COUNT(DISTINCT `FONCT_A_LIGNE`) AS nombreLigne
			...
			AND tc.`POSTE_CYCLE_NOM` = 'immo'
			AND tc.`MISSION_ID` = ".$missionId."
			AND tc.`ENTREPRISE_ID` = ".$entrepriseId."
			AND tf.`FONCT_A_NOM`= 'immo'";

$req = $bdd->query($sql);
$rep = $req->fetch();
if($rep["nombreLigne"] >= 11) {
	$checkLigne = true;
	$result = 1;
}
