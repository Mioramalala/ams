...
$(function(){
	...
	$('#enregistrer_Fermeture_Question_db13').click(function(){	
	if((document.getElementById("commentaire_Question_db13").value=="" && document.getElementById("rad_Question_Non_db_13").checked== true) || (document.getElementById("rad_Question_Oui_db_13").checked==false && document.getElementById("rad_Question_NA_db_13").checked==false && document.getElementById("rad_Question_Non_db_13").checked==false)){
			...
			$('#message_fermeture_question_db13').hide();
		}
		else{
				...
				success:function(e){
					...
					if(objectif_id==0){					
						...
						$.ajax({
							...
							data:{commentaire:commentaire, qcm:qcm, question_id:292},
							success:function(){
								$.ajax({
									type:'POST',
	...
	$('#valider_Fermeture_Question_db13').click(function(){
		$('#message_fermeture_question_db13').hide();
		$('#fancybox_db').hide();
		openButtObjdb();
	});
...
<table width="500">
	...
	<tr align="center" height="50">
		<td>
			<input type="button" class="bouton" value="Oui" id="enregistrer_Fermeture_Question_db13" />&nbsp;&nbsp;&nbsp;
			<input type="button" class="bouton" value="Non" id="valider_Fermeture_Question_db13" />
		</td>
	</tr>
</table>
</div>