...
$(function(){
	...
	$('#enregistrer_Fermeture_Question_stc10').click(function(){
if((document.getElementById("commentaire_Question_stc10").value=="") || (document.getElementById("rad_Question_Oui_stc_10").checked==false && document.getElementById("rad_Question_NA_stc_10").checked==false && document.getElementById("rad_Question_Non_stc_10").checked==false)){
			...
			$('#Question_stc_').hide();
			$('#message_fermeture_question_stc10').hide();
		}
		else{
				...
				success:function(e){
					...
					if(objectif_id==0){					
						...
						$.ajax({
							...
							data:{commentaire:commentaire, qcm:qcm, question_id:144},
							success:function(){
								$.ajax({
									type:'POST',
	...
	$('#valider_Fermeture_Question_stc10').click(function(){
		$('#message_fermeture_question_stc10').hide();
		$('#fancybox_stc').hide();
		openButtObjstc();
	});
...
<table width="500">
	...
	<tr align="center" height="50">
		<td>
			<input type="button" class="bouton" value="Oui" id="enregistrer_Fermeture_Question_stc10" />&nbsp;&nbsp;&nbsp;
			<input type="button" class="bouton" value="Non" id="valider_Fermeture_Question_stc10" />
		</td>
	</tr>
</table>
</div>