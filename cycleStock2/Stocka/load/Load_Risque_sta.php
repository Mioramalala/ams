<script>
	function select_risque_sta(id, poste_id, mission_id, cpt){
		id_risque="risque_sta"+cpt;
		risque=document.getElementById(id_risque).value;
		$.ajax({
			type:'POST',
			url:'cycleStock/Stocka/php/detect_risque_id_existant_sta.php',
			...
			success:function(e){
				...
				if(e==0){
					$.ajax({
						type:'POST',
						url:'cycleStock/Stocka/php/enregistrer_risque_sta.php',
					...
					$.ajax({
						type:'POST',
						url:'cycleStock/Stocka/php/update_risque_sta.php',
...
<table>
	...
	?>	
			<td class="titre" width="100" align="center">
				<select id="risque_sta<?php echo $cpt; ?>" class="verrou_tache16" onchange="select_risque_sta(this.id,<?php echo $donnees['POSTE_CYCLE_ID']; ?>,<?php echo $mission_id;?>,<?php echo $cpt; ?>)" <?php if($conclusionId!=0) echo 'disabled'; ?> >
