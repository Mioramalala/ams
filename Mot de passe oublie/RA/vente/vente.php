<?php
	session_start();
	$_SESSION['objectif']='Ventes-Clients';
...
<html>
	<head>
		...
		$(function(){
				$('#bt_synthese_vente').click(function(){
					...
					$.ajax({
						...
						success:function(e){
							//alert(e);
							document.getElementById("txt_synthese_vente").value=e;
						}
					});
				});
		...
		$(function(){
				$('#bt_conclusion_vente').click(function(){
					...
					$.ajax({
						...
						data:{mission_id:mission_id},
						success:function(e){
							document.getElementById("txt_conclusion_vente").value=e;
						}
					});
				});
		...
		function valider_synthese_vente(){
		 ...
		 function conclusion_venteclient(){
		...
		function addcommentvente(id, numerocompte, compte){
			var comment_vente=$('#commentvente'+compte).val();
			var mission_id=<?php echo $mission_id;?>;
			if(comment_vente!=""){
				$.ajax({
					type:'POST',
					url:'RA/vente/commentairevente.php',
					data:{numerocompte:numerocompte, mission_id:mission_id , comment_vente:comment_vente, objectif:'vente'},
					success:function(){
	...
	<table width="100%" height="50" bgcolor="#00698d" style="text-align:left;">
			<tr>
				<td><center><label class="grand_Titre" style="color:white;">VERIFICATION DE LA VARIATION DES VENTES-CLIENTS</label></center></td>
			</tr>
	</table>
		<div id="bt_milahatra">
			<input type="button" value="Retour" id="id_retour_vente" style="width:80"/>
			<input type="button" value="Synth&egrave;se" id="bt_synthese_vente" style="width:80">
			<input type="button" name="btconclusion" value="Conclusion" id = "bt_conclusion_vente" style="width:80"/>
			<input type ="button" value = "Enregistrer" onclick="enregistrer_tableau_vente();" style="width:80;"/>
		</div><br/>
	...
	<div id="" style="float:left;width:600px;height:500px;">
		<table border="1" width="570">
			...
			<?php
				...
				$reponse=$bdd->query('SELECT IMPORT_ID,IMPORT_COMPTES,IMPORT_INTITULES,IMPORT_DEBIT,IMPORT_CREDIT,IMPORT_SOLDE,IMPORT_CHOIX_ANNEE
					FROM  tab_importer where IMPORT_COMPTES like "7%" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N"');
		...
		<table border="1" width="570">
			...
			include '../../connexion.php';
				...
				$reponse2=$bdd->query('SELECT IMPORT_ID,IMPORT_COMPTES,IMPORT_INTITULES,IMPORT_DEBIT,IMPORT_CREDIT,IMPORT_SOLDE,IMPORT_CHOIX_ANNEE 	FROM tab_importer where IMPORT_COMPTES like "4%" AND IMPORT_COMPTES not like "40%" AND IMPORT_COMPTES not like "42%" AND IMPORT_COMPTES not like "43%" AND IMPORT_COMPTES not like "45%" AND IMPORT_COMPTES not like "46%" AND	IMPORT_COMPTES not like "47%" AND IMPORT_COMPTES not like "49%" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N"');
		...
		<table border="1" width="70%">
			...
			<?php
				$reponse1=$bdd->query('SELECT IMPORT_ID,IMPORT_COMPTES,IMPORT_INTITULES,IMPORT_DEBIT,IMPORT_CREDIT,IMPORT_SOLDE,IMPORT_CHOIX_ANNEE
					FROM  tab_importer where IMPORT_COMPTES like "7%" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N-1"');
				$com3=0;
				$soldedebit='';
				$soldecredit='';
				$soldedebitN1=0;
	...
	<div id="" style="float:right;width:200px;height:500px;position:absolute;top:-220px;left:750px;" />
	<!-------------------------------------------------------variation table 1------------------------------------------------------------------------------>
		<table border="1">
			<tr height="70">
			...
			<tr height="70" id="idBalGen<?php echo $com5;?>" onclick="makaid(<?php echo $donnees['IMPORT_ID'];?>)" value="<?php echo $donnees['IMPORT_ID'];?>">
				...
				<td width="80">
					<textarea cols="5" rows="1" id="commentvente<?php echo $com5 ?>"></textarea>
					<img src="../RA/image/add.jpg" id="com5<?php echo $com5; ?>" onclick="addcommentvente(this.id,<?php echo $numerodecomptefin[$i]; ?>,<?php echo $com5;?>)" width="20" height="20"/>
				</td>
				<td width="80"><input type="button" id="btn<?php echo $com5;?>" onclick="addRenvoievente(this.id, <?php echo $com5;?>)" value="ok" /></td>
			</tr>
			
			<div id="id_renvoie_vente" style="position:absolute;top:180px;left:392px;background-color:#ccc;width:300px;height:200px;display:none">
				<form method = "post" enctype="multipart/form-data" action = "RA/vente/importvente.php?mission_id=<?php echo $mission_id;?>&importid=<?php echo $numerodecomptefin[$i]; ?>">
					<input type = "file" accept="application/vnd.ms-excel" id = "file_Import_vente" name = "file_Import_vente"/>
					<input type="submit" name="Importervente" value="Upload" id = "id_Importervente"/>
					<?php
						$reponse2=$bdd->query('select RENVOIE_LIEN from tab_renvoie where MISSION_ID='.$mission_id.' AND RENVOIE_OBJECTIF="vente" AND IMPORT_ID='.$numerodecomptefin[$i]);					
						$renvoielien='';
						while($donnees2=$reponse2->fetch()){
				...
				<div id="fermeture_import_vente" style="top:-18px;left:280px;position:absolute;"><img src="../cycle_achat_image/Fermer.png" /></div>
			</div> 
			<?php
				$com5++;
				}
		...
		<table border="1">
			...
			<tr height="70" id="idBalGen<?php echo $com6;?>" onclick="makaid(<?php echo $donnees4['IMPORT_ID'];?>)" value="<?php echo $donnees4['IMPORT_ID'];?>">
				...
				<td width="80">
					<textarea cols="5" rows="1" id="commentvente<?php echo $com6 ?>"></textarea>
					<img src="../RA/image/add.jpg" id="com6<?php echo $com6; ?>" onclick="addcommentvente(this.id,<?php echo $numerodecomptefin6[$i]; ?>,<?php echo $com6;?>)" width="20" height="20"/>
				</td>
				<td width="80"><input type="button" id="btn<?php echo $com6;?>" onclick="addRenvoievente(this.id, <?php echo $com6;?>)" value="ok" /></td>
			</tr>
			
			<div id="id_renvoie_vente" style="position:absolute;top:180px;left:392px;background-color:#ccc;width:300px;height:200px;display:none">
				<form method = "post" enctype="multipart/form-data" action = "RA/vente/importvente.php?mission_id=<?php echo $mission_id;?>&importid=<?php echo $numerodecomptefin6[$i]; ?>">
					<input type = "file" accept="application/vnd.ms-excel" id = "file_Import_vente" name = "file_Import_vente"/>
					<input type="submit" name="vente" value="Upload" id = "id_Importervente"/>
					<?php
						$reponse2=$bdd->query('select RENVOIE_LIEN from tab_renvoie where MISSION_ID='.$mission_id.' AND RENVOIE_OBJECTIF="vente" AND IMPORT_ID='.$numerodecomptefin6[$i]);					
						$renvoielien='';
						while($donnees2=$reponse2->fetch()){
							$renvoielien=$donnees2['RENVOIE_LIEN'];
						}
...
<div id="centre_vente" align="center">
		...
		<table width="500" bgcolor="#00698d">
				<td class="td_Titre_Question" align="center"><textarea id="txt_synthese_vente" cols="40" rows="20" <?php// if($conclusionvente!=0) echo 'disabled'; ?> ></textarea>
				</td>
			</tr>
			<tr>
				<td align="center">
					<input type="button" id="bt_valider_synthese_vente" value="Enregistrer" class="bouton" onclick="valider_synthese_vente();" <?php //if($conclusionvente!=0) echo 'disabled'; ?> />&nbsp;&nbsp;&nbsp;
		...
		<div id="dv_conclusion_vente" data-options="handle:'#dragg_c'" align="center" style="display:none;position:absolute;top:150px;left:350px;width:500px;height:395px;background-color:#05abe1;border:5px solid #ccc;">
			...
			<table width="500" bgcolor="#00698d">
					<td class="td_Titre_Question" align="center"><textarea id="txt_conclusion_vente" cols="40" rows="20" <?php// if($conclusionvente!=0) echo 'disabled'; ?>></textarea>
					</td>
				</tr>
				<tr>
					<td align="center">
						<input type="button" id="bt_valider_conclusion_vente" value="Valider" class="bouton" onclick="conclusion_venteclient();"  <?php// if($conclusionvente!=0) echo 'disabled'; ?>>&nbsp;&nbsp;&nbsp;
