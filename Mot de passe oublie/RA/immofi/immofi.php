<?php
	session_start();
	$_SESSION['objectif']='Immobilisations financieres';
...
<html>
	<head>
		...
		$(function(){
				$('#bt_conclusion_immofi').click(function(){
					...
					$.ajax({
						...
						data:{mission_id:mission_id},
						success:function(e){
							document.getElementById("txt_conclusion_immofi").value=e;
						}
					});
				});
		...
		function valider_synthese_immofi(){
		 ...
		 function conclusion_immofinancier(){
		...
		function addcommentimmofi(id, numerocompte, compte){
			var comment_immofi=$('#commentimmofi'+compte).val();
			var mission_id=<?php echo $mission_id;?>;
			if(comment_immofi!=""){
				$.ajax({
					type:'POST',
					url:'RA/immofi/commentaireimmofi.php',
					data:{numerocompte:numerocompte, mission_id:mission_id , comment_immofi:comment_immofi, objectif:'immofi'},
					success:function(){
			...
			var i = 0;
				for(i=0;i<3;i++){
					...
					$.ajax({
							...
							n:mission_id},
								success:function(e){
									 alert('Immobilisations financieres bien enregistr\351es');	
	...
	?>
		<table width="100%" height="50" bgcolor="#00698d" style="text-align:left;">
			<tr>
				<td><center><label class="grand_Titre" style="color:white;">VERIFICATION DE LA VARIATION DES IMMOBILISATIONS FINANCIERES</label></center></td>
			</tr>
		</table>
		<div id="bt_milahatra">
			<input type="button" value="Retour" id="id_retour_immofi" style="width:80"/>
			<input type="button" value="Synth&egrave;se" id="bt_synthese_immofi" style="width:80">
			<input type="button" name="btconclusion" value="Conclusion" id = "bt_conclusion_immofi" style="width:80"/>
			<input type ="button" value = "Enregistrer" onclick="enregistrer_tableau_immofi();" style="width:80;"/>
		</div><br/>
	...
	<div id="" style="float:left;width:600px;height:500px;">
		<table border="1" width="570"">
			...
			<?php
				...
				$soldefinaldebit=0;
				$numerodecompte='';
				$reponse=$bdd->query('SELECT IMPORT_ID,IMPORT_COMPTES,IMPORT_INTITULES,IMPORT_DEBIT,IMPORT_CREDIT,IMPORT_SOLDE,IMPORT_CHOIX_ANNEE FROM  tab_importer where IMPORT_COMPTES like "2%" AND IMPORT_COMPTES not like "20%" AND IMPORT_COMPTES not like "21%" AND IMPORT_COMPTES not like "22%" AND IMPORT_COMPTES not like "23%" AND IMPORT_COMPTES not like "24%" AND IMPORT_COMPTES not like "25%" AND IMPORT_COMPTES not like "28%" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N"');
	...
	<div id="" style="float:left;width:200px;height:500px;">
		<table border="1" width="70%">
			...
			?>
			<tr height="70" id="idBalGen<?php echo $com2;?>" onclick="makaid(<?php echo $donnees['IMPORT_ID'];?>)" value="<?php echo $donnees['IMPORT_ID'];?>">
	...
	<div id="" style="float:right;width:260px;height:500px;position:absolute;top:-220px;left:750px;" />
		<table border="1">
			...
			<tr height="70" id="idBalGen<?php echo $com3;?>" onclick="makaid(<?php echo $donnees['IMPORT_ID'];?>)" value="<?php echo $donnees['IMPORT_ID'];?>">
				...
				<td width="90">
					<textarea cols="5" rows="1" id="commentimmofi<?php echo $com3 ?>"></textarea>
					<img src="../RA/image/add.jpg" id="com3<?php echo $com3; ?>" onclick="addcommentimmofi(this.id,<?php echo $numerodecomptefin[$i]; ?>,<?php echo $com3;?>)" width="20" height="20"/>
				</td>
				<td width="90"><input type="button" id="btn<?php echo $com3;?>" onclick="addRenvoieimmofi(this.id, <?php echo $com3;?>)" value="ok" /></td>
			</tr>
			
			<div id="id_renvoie_immofi" style="position:absolute;top:180px;left:392px;background-color:#ccc;width:300px;height:200px;display:none">
				<form method = "post" enctype="multipart/form-data" action = "RA/immofi/importimmofi.php?mission_id=<?php echo $mission_id;?>&importid=<?php echo $numerodecomptefin[$i]; ?>">
					<input type = "file" accept="application/vnd.ms-excel" id = "file_Import_immofi" name = "file_Import_immofi"/>
					<input type="submit" name="Importerimmofi" value="Upload" id = "id_Importerimmofi"/>
					<?php
						$reponse2=$bdd->query('select RENVOIE_LIEN from tab_renvoie where MISSION_ID='.$mission_id.' AND RENVOIE_OBJECTIF="immofi" AND IMPORT_ID='.$numerodecomptefin[$i]);					
						$renvoielien='';
				...
				<div id="fermeture_import_immofi" style="top:-18px;left:280px;position:absolute;"><img src="../cycle_achat_image/Fermer.png" /></div>
			</div> 
			<?php
				$com3++;
				}
...
<div id="centre_immofi" align="center">
		...
		<table width="500" bgcolor="#00698d">
				<td class="td_Titre_Question" align="center"><textarea id="txt_synthese_immofi" cols="40" rows="20" <?php// if($conclusionimmofi!=0) echo 'disabled'; ?> ></textarea>
				</td>
			</tr>
			<tr>
				<td align="center">
					<input type="button" id="bt_valider_synthese_immofi" value="Enregistrer" class="bouton" onclick="valider_synthese_immofi();" <?php //if($conclusionimmofi!=0) echo 'disabled'; ?> />&nbsp;&nbsp;&nbsp;
		...
		<div id="dv_conclusion_immofi" data-options="handle:'#dragg_c'" align="center" style="display:none;position:absolute;top:150px;left:350px;width:500px;height:395px;background-color:#05abe1;border:5px solid #ccc;">
			...
			<table width="500" bgcolor="#00698d">
					<td class="td_Titre_Question" align="center"><textarea id="txt_conclusion_immofi" cols="40" rows="20" <?php// if($conclusionimmofi!=0) echo 'disabled'; ?>></textarea>
					</td>
				</tr>
				<tr>
					<td align="center">
						<input type="button" id="bt_valider_conclusion_immofi" value="Valider" class="bouton" onclick="conclusion_immofinancier();"  <?php// if($conclusionimmofi!=0) echo 'disabled'; ?>>&nbsp;&nbsp;&nbsp;
