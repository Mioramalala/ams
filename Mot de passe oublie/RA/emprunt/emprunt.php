<?php
	session_start();
	$_SESSION['objectif']='Emprunts et dettes ';
...
<html>
	...
	<table width="100%" height="50" bgcolor="#00698d" style="text-align:left;">
		...
		<div id="bt_milahatra">
			...
			<input type ="button" value = "Enregistrer" onclick="enregistrer_commentaire_emprunt();" style="width:80;"/>
		</div><br/>
			<label id = "dv_labelako">P&eacute;riode du:<b><?php echo ' '.$mission_debut.' ';?></b> au<b> <?php echo ' '.$mission_fin;?></b> </label><br/><br/><br/>
			<br />
		
		<label style="position:relative;left:50px;"><?php echo ' '.$mission_annee.' '?></label>
		<label style="position:relative;left:200px;"><?php echo ' '.$mission_anneeN1.' '?></label>
	...
	<div id="" style="float:left;width:600px;height:500px;">
		...
		<?php
				...
				$soldefinaldebit=0;
				$numerodecompte='';
				$reponse=$bdd->query('SELECT IMPORT_ID,IMPORT_COMPTES,IMPORT_INTITULES,IMPORT_DEBIT,IMPORT_CREDIT,IMPORT_SOLDE,IMPORT_CHOIX_ANNEE FROM  tab_importer where IMPORT_COMPTES like "66%" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N"');
		...
		<!-------------------------------------------------------like16 N------------------------------------------------------------------------------>
			<?php
				...
				$reponse2=$bdd->query('SELECT IMPORT_ID,IMPORT_COMPTES,IMPORT_INTITULES,IMPORT_DEBIT,IMPORT_CREDIT,IMPORT_SOLDE,IMPORT_CHOIX_ANNEE 
					FROM  tab_importer where IMPORT_COMPTES like "16%" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N"');
	...
	<!-------------------------------------------------------like66 N-1------------------------------------------------------------------------------>
			<?php
				...
				FROM  tab_importer where IMPORT_COMPTES like "66%" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N-1"');
				$com3=0;
				$soldedebit='';
				$soldecredit='';
				$soldedebitN1=0;
		...
		<!-------------------------------------------------------like16 N------------------------------------------------------------------------------>
			<?php
				$reponse4=$bdd->query('SELECT IMPORT_ID,IMPORT_COMPTES,IMPORT_INTITULES,IMPORT_DEBIT,IMPORT_CREDIT,IMPORT_SOLDE,IMPORT_CHOIX_ANNEE
					FROM  tab_importer where IMPORT_COMPTES like "16%" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N-1"');
				$com4=0;
				$soldedebit61='';
				$soldecredit61='';
				$soldedebitN61=0;
	...
	<div id="" style="float:right;width:260px;height:500px;position:absolute;top:-220px;left:750px;" />
		...
		<!----------------------------------------------------------------------Variation table1----------------------------------------------------------->
			...
			<tr height="70" id="idBalGen<?php echo $com5;?>" onclick="makaid(<?php echo $donnees['IMPORT_ID'];?>)" value="<?php echo $donnees['IMPORT_ID'];?>">
				...
				<td width="90">
					<textarea cols="5" rows="1" id="commentemprunt<?php echo $com5 ?>"></textarea>
					<img src="../RA/image/add.jpg" id="com5<?php echo $com5; ?>" onclick="addcommentemprunt(this.id,<?php echo $numerodecomptefin[$i]; ?>,<?php echo $com5;?>)" width="20" height="20"/>
				</td>
				<td width="90"><input type="button" id="btn<?php echo $com5;?>" onclick="addRenvoieemprunt(this.id, <?php echo $com5;?>)" value="ok" /></td>
			</tr>
			
			<div id="id_renvoie_emprunt" style="position:absolute;top:180px;left:392px;background-color:#ccc;width:300px;height:200px;display:none">
		...
		<!----------------------------------------------------------------------Variation table2----------------------------------------------------------->
			...
			<tr height="70" id="idBalGen<?php echo $com6;?>" onclick="makaid(<?php echo $donnees4['IMPORT_ID'];?>)" value="<?php echo $donnees4['IMPORT_ID'];?>">
				...
				<td width="90">
					<textarea cols="5" rows="1" id="commentemprunt<?php echo $com6 ?>"></textarea>
					<img src="../RA/image/add.jpg" id="com<?php echo $com6; ?>" onclick="addcommentemprunt(this.id,<?php echo $numerodecomptefin6[$i]; ?>,<?php echo $com6;?>)" width="20" height="20"/>
				</td>
				<td width="90"><input type="button" id="btn<?php echo $com6;?>" onclick="addRenvoieemprunt(this.id, <?php echo $com6;?>)" value="ok" /></td>
			</tr>
			
			<div id="id_renvoie_emprnut" style="position:absolute;top:180px;left:392px;background-color:#ccc;width:300px;height:200px;display:none">
				<form method = "post" enctype="multipart/form-data" action = "RA/emprunt/emprunt.php?mission_id=<?php echo $mission_id;?>&importid=<?php echo $numerodecomptefin6[$i]; ?>">
					<input type = "file" accept="application/vnd.ms-excel" id = "file_Import_emprunt" name = "file_Import_emprunt"/>
					<input type="submit" name="Importeremprunt" value="Upload" id = "id_Importeremprunt"/>
					<?php
						$reponse2=$bdd->query('select RENVOIE_LIEN from tab_renvoie where MISSION_ID='.$mission_id.' AND RENVOIE_OBJECTIF="emprunt" AND IMPORT_ID='.$numerodecomptefin6[$i]);					
						$renvoielien='';
						while($donnees2=$reponse2->fetch()){
							$renvoielien=$donnees2['RENVOIE_LIEN'];
						}
...
<div id="centre_emprunt" align="center">
		...
		<table width="500" bgcolor="#00698d">
				<td class="td_Titre_Question" align="center"><textarea id="txt_synthese_emprunt" cols="40" rows="20" <?php// if($conclusionemprunt!=0) echo 'disabled'; ?> ></textarea>
				</td>
			</tr>
			<tr>
				<td align="center">
					<input type="button" id="bt_valider_synthese_emprunt" value="Enregistrer" class="bouton" onclick="valider_synthese_emprunt();" <?php //if($conclusionemprunt!=0) echo 'disabled'; ?> />&nbsp;&nbsp;&nbsp;
		...
		<div id="dv_conclusion_emprunt" data-options="handle:'#dragg_c'" align="center" style="display:none;position:absolute;top:150px;left:350px;width:500px;height:395px;background-color:#05abe1;border:5px solid #ccc;">
			...
			<table width="500" bgcolor="#00698d">
				...
				<tr>
					<td align="center">
						<input type="button" id="bt_valider_conclusion_emprunt" value="Valider" class="bouton" onclick="conclusion_empruntdette();"  <?php// if($conclusionemprunt!=0) echo 'disabled'; ?>>&nbsp;&nbsp;&nbsp;
