...
$reponse=$bdd->query('SELECT SYNTHESE_ID,SYNTHESE_RISQUE,SYNTHESE_COMMENTAIRE,SCORE FROM tab_synthese WHERE MISSION_ID= "'.$mission_id.'" AND CYCLE_ACHAT_ID=20');
...
if($donnees){
	...
	if($donnees['SYNTHESE_ID']!=0){
		$req='UPDATE tab_synthese SET 	SYNTHESE_COMMENTAIRE="'.$commentaire.'", 
										...
										SCORE="'.$echo_score_rd.'", 
										UTIL_ID='.$UTIL_ID.' WHERE CYCLE_ACHAT_ID=20';
		echo $req;
	...
	$reponse1 = $bdd->exec('INSERT INTO tab_synthese (
