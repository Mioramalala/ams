...
$(function(){
	...
	$('#enregistrer_Fermeture_Question_rd4').click(function(){	
	if((document.getElementById("commentaire_Question_rd4").value=="" && document.getElementById("rad_Question_Non_rd_4").checked== true) || (document.getElementById("rad_Question_Oui_rd_4").checked==false && document.getElementById("rad_Question_NA_rd_4").checked==false && document.getElementById("rad_Question_Non_rd_4").checked==false)){
			...
			$('#message_fermeture_question_rd4').hide();
			...
			$.ajax({
				...
				success:function(e){
					objectif_id=e;
					if(objectif_id==0){					
						enregistrer_commentaire_f(commentaire, qcm, mission_id, 267);
					}
					else{
						$.ajax({
							type:'POST',
							url:'cycleRecette/Recetted/php/update_object_rd.php',
							data:{commentaire:commentaire, qcm:qcm, question_id:267},
							success:function(){
								$.ajax({
									type:'POST',
	...
	$('#valider_Fermeture_Question_rd4').click(function(){
		$('#message_fermeture_question_rd4').hide();
		$('#fancybox_rd').hide();
		openButtObjrd();
	});
...
<table width="500">
	...
	<tr align="center" height="50">
		<td>
			<input type="button" class="bouton" value="Oui" id="enregistrer_Fermeture_Question_rd4" />&nbsp;&nbsp;&nbsp;
			<input type="button" class="bouton" value="Non" id="valider_Fermeture_Question_rd4" />
		</td>
	</tr>
</table>
</div>