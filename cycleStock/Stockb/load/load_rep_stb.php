<?php  
	...
	$verif_=$bdd->query("SELECT count(*) as nb FROM tab_conclusion WHERE CYCLE_ACHAT_ID='11' AND MISSION_ID='$mission_id'");
...
$(function(){
	...
	var validStockB= parseInt(<?php echo $validStockB;?>);
	if(validStockB==1) {
		alert('misy conclusion load rep');
		$('textarea').attr('disabled',true);
		$(':input[type=radio]').attr('disabled',true);
		$('#Synthese_stb_Terminer').attr('disabled',true);
	};
