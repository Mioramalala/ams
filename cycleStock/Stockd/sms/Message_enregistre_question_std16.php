...
$(function(){
	...
	$('#enregistrer_Fermeture_Question_std16').click(function(){
if((document.getElementById("commentaire_Question_std16").value=="") || (document.getElementById("rad_Question_Oui_std_16").checked==false && document.getElementById("rad_Question_NA_std_16").checked==false && document.getElementById("rad_Question_Non_std_16").checked==false)){
			...
			$('#message_fermeture_question_std16').hide();
		}
		else{
				...
				success:function(e){
					...
					if(objectif_id==0){					
						...
						$.ajax({
							...
							data:{commentaire:commentaire, qcm:qcm, question_id:167},
							success:function(){
								$.ajax({
									type:'POST',
	...
	$('#valider_Fermeture_Question_std16').click(function(){
		$('#message_fermeture_question_std16').hide();
		$('#fancybox_std').hide();
		openButtObjstd();
	});
...
<table width="500">
	...
	<tr align="center" height="50">
		<td>
			<input type="button" class="bouton" value="Oui" id="enregistrer_Fermeture_Question_std16" />&nbsp;&nbsp;&nbsp;
			<input type="button" class="bouton" value="Non" id="valider_Fermeture_Question_std16" />
		</td>
	</tr>
</table>
</div>