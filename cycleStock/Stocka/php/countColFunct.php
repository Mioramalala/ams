...
//check ligne : verifie si chaqhe ligne de la separation de fonction est checkée, pour stocka il y a 13 lignes à cocher
$sql= "SELECT COUNT(DISTINCT `FONCT_A_LIGNE`) AS nombreLigne
			...
			AND tc.`POSTE_CYCLE_NOM` = 'stock'
			AND tc.`MISSION_ID` = ".$missionId."
			AND tc.`ENTREPRISE_ID` = ".$entrepriseId."
			AND tf.`FONCT_A_NOM`= 'stock'";

$req = $bdd->query($sql);
$rep = $req->fetch();
 
if($rep["nombreLigne"] >= 13) {
	$checkLigne = true;
	$result= 1;
}
...
if ($checkLigne) {
	...
	while($donnees= $reponse->fetch()){
		...
		//$result= $result .'_' .$donnees['COMPTE'];
	}
