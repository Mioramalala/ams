...
function compte_cpt9(id, poste_id, cpt, mission_id){
	...
	if(document.getElementById(id_9).checked==true){
		...
		$.ajax({
			...
			data:{poste_id:poste_id, ligne:9},
			success:function(){
			}
		});		
	}
...
{
$reponse1=$bdd->query('SELECT FONCT_ACHAT_A_RESULT FROM tab_fonction_achat_a WHERE POSTE_CYCLE_ID='.$donnees['POSTE_CYCLE_ID'].' AND FONCT_ACHAT_A_LIGNE=9');

$donnees1=$reponse1->fetch();

?>	
		<td class="titre" width="100" align="center">
			<input type="checkbox" onchange="compte_cpt9(this.id,<?php echo $donnees ['POSTE_CYCLE_ID']; ?>,<?php echo $cpt_col; ?>, <?php echo $mission_id; ?>)" id="id_9<?php echo $cpt_col; ?>" <?php if($donnees1['FONCT_ACHAT_A_RESULT']==1) echo 'checked';?>/>
			<?php
				$cpt_col++;
			?>
		</td>
