<?php
	session_start();
	$_SESSION['objectif']='Paie et Personnel';
...
<html>
			...
			$.ajax({
				...
				data:{synthese_paie:synthese_paie,mission_id:mission_id},
					success:function(e){
						...
						// waiting();$('#contenue').load('RA/paie/paie.php?mission_id='+<?php echo $mission_id; ?>,stopWaiting);	
					}
			});
		}
		 
		 function conclusion_paie1(){
		...
		function enregistrer_tableau_paie(){
			var i = 0;
				for(i=0;i<<?php echo $a;?>;i++){
					...
					var enregistrer_tableau_import13=$('#commentpaie'+i).val();
	...
	<table width="100%" height="50" bgcolor="#00698d" style="text-align:left;">
			<tr>
				<td><center><label class="grand_Titre" style="color:white;">VERIFICATION DE LA VARIATION DE PAIE ET PERSONNEL</label></center></td>
			</tr>
	</table>
		<div id="bt_milahatra">
			...
			<input type ="button" value = "Enregistrer" id="enregistrer_page" onclick="enregistrer_tableau_paie();" style="width:80;"/>
		</div><br/>
	...
	<div id="" style="overflow:auto;width:1200px;height:350px;" >
		<table class="table_parcycle" width="600px">
			<?php
				...
				$reponse=$bdd->query('SELECT IMPORT_ID,IMPORT_COMPTES,IMPORT_INTITULES,IMPORT_DEBIT,IMPORT_CREDIT,IMPORT_SOLDE,IMPORT_CHOIX_ANNEE 
				FROM  tab_importer 
				where IMPORT_CYCLE="H- Paie et Personnel" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N"');
		...
		<table  class="table_parcycle" width="100px">
			<?php
				...
				FROM  tab_importer 
				where IMPORT_CYCLE="H- Paie et Personnel" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N-1"');
				$com2=0;
				$soldedebit='';
				$soldecredit='';
				$soldedebitN1=0;
		...
		<table  class="table_parcycle" width="482px">
			...
			<tr height="70" id="idBalGen<?php echo $com3;?>" onclick="makaid(<?php echo $donnees['IMPORT_ID'];?>)" value="<?php echo $donnees['IMPORT_ID'];?>">
				...
				<td width="200px">
					<textarea cols="5" rows="1" id="commentpaie<?php echo $com3 ?>"></textarea>
					<img src="../RA/image/add.jpg" id="com3<?php echo $com3; ?>" onclick="addcommentpaie(this.id,<?php echo $numerodecomptefin[$i]; ?>,<?php echo $com;?>)" width="20" height="20"/>
				</td>
				<td width="100px"><input type="button" id="btn<?php echo $com3;?>" onclick="addRenvoiepaie(this.id, <?php echo $com3;?>)" value="ok" /></td>
			</tr>
			
			<div id="id_renvoie_paie" style="position:absolute;top:180px;left:392px;background-color:#ccc;width:300px;height:100px;display:none">
...
<div id="centre_paie" align="center">
		...
		<div id="dv_conclusion_paie" data-options="handle:'#dragg_c'" align="center" style="display:none;position:absolute;top:150px;left:350px;width:500px;height:395px;background-color:#05abe1;border:5px solid #ccc;">
			...
			<table width="500" bgcolor="#00698d">
				...
				<tr>
					<td align="center">
						<input type="button" id="bt_valider_conclusion_paie" value="Valider" class="bouton" onclick="conclusion_paie1();">&nbsp;&nbsp;&nbsp;
