...
<html>
		...
		<script>
			$(function(){
				$("#renvoi").click(function(){
					waiting();$("#ChampGauche").load("RDC/emprunt/empruntC/TabC1.php",stopWaiting);
					$('#ChampDroite').show();
					$('#suivantIT').show();
					$('#precedIT').show();
					$('#retourIT').hide();
				...
				$("#suivantIT").click(function(){
					...
					if((rep1=="non" && cmt1=="") || (rep2=="non" && cmt2=="") || rep1=="" || rep2==""){ 
						...
						for(var i=0; i<(lignes); i++){
							...
							var solde = $('#solde_'+i).val();
							var soldeComp = $('#soldeComp_'+i).val();
							var difference = $('#difference_'+i).val();		
							var observation = $('#observation_'+i).val();	
							if(soldeDev!='' || devise!='' || cloture!='' || solde!='' || soldeComp!='' || difference!='' || observation!='')
							{add_Input(compte, soldeDev, devise, cloture, solde, soldeComp, difference, observation, <?php echo $mission_id; ?>);}					
						}
				...
				$("#precedIT").click(function(){
					waiting();$("#contenue").load("RDC/emprunt/empruntC/objetCED1.php",stopWaiting);
				});
			});

	function add_Input(compte, soldeDev, devise, cloture, solde, soldeComp, difference, observation, mission_id){
		$.ajax({
			type:'POST',
			url:'RDC/emprunt/empruntC/add_Input_C1.php',
			data:{compte:compte,soldeDev:soldeDev,devise:devise, cloture:cloture, solde:solde, 
	...
	<body>
		<div id="GranTitre">
			C - EVALUATION DES SOLDES : Partie I	
		</div>		
		<div id="ChampGauche">
