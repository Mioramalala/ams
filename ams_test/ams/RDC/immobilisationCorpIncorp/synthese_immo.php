...
function disableButtons() {
	document.getElementById('id_enregistrer_immo').disabled      = true;
	document.getElementById('bt_valider_synthese_immo').disabled = true;
...
$(function(){
		...
		getValidationSynthese('immo', <?php echo $mission_id;?>, function(e) {
			if(e === 'validee') {
				disableButtons();
			}
		});
	...
	function enregistrer_immo(){
			...
			$.ajax({
				type:'GET',
				url:'RDC/immobilisationCorpIncorp/tableau_immo.php',
				data:datatransfert,
					success:function(e){
						//alert(e);
	...
	$(function(){
		$('#id_retour_immo').click(function(){
			waiting();$('#contenue').load('RDC/immobilisationCorpIncorp/feuille_maitresse.php?mission_id='+<?php echo $mission_id; ?>,stopWaiting);		
		});
		
		$('#bt_validation_immo').click(function(){
			$('#dv_synthese_immo').fadeIn();
			getSynthese('immo', <?php echo $mission_id;?>, function(e) {
	...
	function valider_synthese_immo(){
			...
			validerSynthese('immo', <?php echo $mission_id;?>, synthese_immo, function(e) {
				$('#dv_synthese_immo').hide();
				alert('Synth\350ses bien enregistr\351es');
				disableButtons();
			});
...
<form method="post"  id="frmenvoyersynthese">
	<table style="text-align:center;background-color:#00698d;color:white;width:100%;padding-top:5px;">
		<tr>
			<td><label>SYNTHESE IMMOBILISATIONS CORPORELLES ET INCORPORELLES</label></td>
		</tr>
	</table>
	
		<div id="bt_milahatra">
				<input type="button" value="Retour" id="id_retour_immo" style="width:100px;height:30px;float:right"/><br/><br/>
				<input type="button" value="Enregistrer" id="id_enregistrer_immo" onclick="enregistrer_immo()"  style="width:100px;height:30px;float:right"/><br/><br/>
				<input type="button" value="Validation" id="bt_validation_immo" style="width:100px;height:30px;float:right"><br/>
		 ...
		 <div class="cadre">
			<table id="tabB2">
				...
				<?php
					...
					$reponse=$bdd->query('select * FROM tab_synthese_feuille_rdc where SYNTHESE_CYCLE="immo" AND  MISSION_ID='.$mission_id);
					(int)$ligne=1;
		...
		<div id="dv_synthese_immo" data-options="handle:'#dragg_c'" align="center" style="display:none;position:absolute;top:150px;left:350px;width:500px;height:395px;background-color:#05abe1;border:5px solid #ccc;">
			<div id="dragg_c" class="td_Titre_Question"><br />SYNTHESE</div>
				<table width="500" bgcolor="#00698d">
					<tr>
						<td class="td_Titre_Question" align="center"><textarea id="txt_synthese_immo" cols="40" style=" width:400px;height:300px"></textarea></td>
					</tr>
					<tr>
						<td align="center">
							<input type="button" id="bt_valider_synthese_immo" value="Enregistrer" class="bouton" onclick="valider_synthese_immo();"/>&nbsp;&nbsp;&nbsp;
