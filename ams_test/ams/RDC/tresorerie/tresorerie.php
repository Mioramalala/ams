...
<html>
			...
			$.ajax({
				...
				data:{synthese_tres:synthese_tres,mission_id:mission_id},
					success:function(e){
						...
						// waiting();$('#contenue').load('RA/tresorerie/tresorerie.php?mission_id='+<?php echo $mission_id; ?>,stopWaiting);	
					}
			});
		}
		...
		function enregistrer_tableau_tres(){
			var i = 0;
				for(i=0;i<<?php echo $a;?>;i++){
					...
					var enregistrer_tableau_import13=$('#commenttres'+i).val();
	...
	<table width="100%" height="50" bgcolor="#00698d" style="text-align:left;">
		...
		<div id="bt_milahatra">
			...
			<input type ="button" value = "Enregistrer" id="enregistrer_page" onclick="enregistrer_tableau_tres();" style="width:80;"/>
		</div><br/>
	...
	<div id="" style="overflow:auto;width:1200px;height:350px;" >
		<table class="table_parcycle" width="600px">
			<?php
				...
				$reponse=$bdd->query('SELECT IMPORT_ID,IMPORT_COMPTES,IMPORT_INTITULES,IMPORT_DEBIT,IMPORT_CREDIT,IMPORT_SOLDE,IMPORT_CHOIX_ANNEE 
				FROM  tab_importer 
				where IMPORT_CYCLE="E- Tr&eacute;soreries" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N"');
		...
		<table class="table_parcycle" width="100px">
			<?php
				...
				FROM  tab_importer 
				where IMPORT_CYCLE="E- Tr&eacute;soreries" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N-1"');
				$com2=0;
				$soldedebit='';
				$soldecredit='';
				$soldedebitN1=0;
		...
		<table  class="table_parcycle" width="482px">
			...
			<tr height="70" id="idBalGen<?php echo $com3;?>" onclick="makaid(<?php echo $donnees['IMPORT_ID'];?>)" value="<?php echo $donnees['IMPORT_ID'];?>">
				...
				<td width="200px">
					...
					<img src="../RA/image/add.jpg" id="com3<?php echo $com3; ?>" onclick="addcommenttres(this.id,<?php echo $numerodecomptefin[$i]; ?>,<?php echo $com;?>)" width="20" height="20"/>
				</td>
				<td width="100px"><input type="button" id="btn<?php echo $com3;?>" onclick="addRenvoietres(this.id, <?php echo $com3;?>)" value="ok" /></td>
			</tr>
			
			<div id="id_renvoie_tres" style="position:absolute;top:180px;left:392px;background-color:#ccc;width:300px;height:100px;display:none">
