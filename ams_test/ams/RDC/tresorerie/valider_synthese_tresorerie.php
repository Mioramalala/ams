...
include '../../connexion.php';
	...
	$synthese_tresorerie=$_POST['synthese_tresorerie'];
	...
	if($donnees0['SYNTHESE_ID_RDC']!=0){
		$reponse1=$bdd->exec('update tab_validation_synthese_rdc set UTIL_ID = "'.$UTIL_ID.'",	SYNTHESE_RDC="'.$synthese_tresorerie.'" where MISSION_ID="'.$mission_id.'" AND SYNTHESE_ID_RDC='.$donnees0['SYNTHESE_ID_RDC']);
		if(!$reponse1) echo  'update tab_validation_synthese_rdc set UTIL_ID = "'.$UTIL_ID.'",SYNTHESE_RDC="'.$synthese_tresorerie.'" where MISSION_ID="'.$mission_id.'" AND SYNTHESE_ID_RDC='.$donnees0['SYNTHESE_ID_RDC'];
	}
	else{
		$reponse=$bdd->exec('INSERT INTO tab_validation_synthese_rdc(SYNTHESE_RDC,SYNTHESE_CYCLE_RDC,MISSION_ID,UTIL_ID) VALUE 
		("'.$synthese_tresorerie.'","tresorerie","'.$mission_id.'","'.$UTIL_ID.'")');
	}
