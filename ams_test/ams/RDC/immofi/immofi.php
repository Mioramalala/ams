<?php
	session_start();
	$_SESSION['objectif']='Immobilisation financières';
...
<html>
			...
			$.ajax({
				...
				data:{synthese_immofi:synthese_immofi,mission_id:mission_id},
					success:function(e){
						...
						// waiting();$('#contenue').load('RA/immofi/immofi.php?mission_id='+<?php echo $mission_id; ?>,stopWaiting);	
					}
			});
		}
		...
		//alert("meme");
			var i = 0;
				for(i=0;i<<?php echo $a;?>;i++){
					...
					var enregistrer_tableau_import13=$('#commentimmofi'+i).val();
					...
					$.ajax({
							...
							n:mission_id},
								success:function(e){
									// alert('Immobilisations financi\350res bien enregistr\351s');
	...
	<table width="100%" height="50" bgcolor="#00698d" style="text-align:left;">
		...
		<div id="bt_milahatra">
			...
			<input type ="button" value = "Enregistrer" id="enregistrer_page" onclick="enregistrer_tableau_immofi();" style="width:80;"/>
		</div><br/>
	...
	<div id="" style="overflow:auto;width:1200px;height:350px;">	
		<table class="table_parcycle" width="600px">
			<?php
				...
				FROM  tab_importer 
				where IMPORT_CYCLE="C- Immobilisations financi&egrave;res" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N"');
		...
		<table class="table_parcycle" width="100px">
			<?php
				...
				FROM  tab_importer 
				where IMPORT_CYCLE="C- Immobilisations financi&egrave;res" AND MISSION_ID='.$mission_id.' AND IMPORT_CHOIX_ANNEE="N-1"');
				$com2=0;
				$soldedebit='';
				$soldecredit='';
				$soldedebitN1=0;
		...
		<table  class="table_parcycle" width="482px">
			...
			<tr height="70" id="idBalGen<?php echo $com3;?>" onclick="makaid(<?php echo $donnees['IMPORT_ID'];?>)" value="<?php echo $donnees['IMPORT_ID'];?>">
				...
				<td width="200px">
					...
					<img src="../RA/image/add.jpg" id="com3<?php echo $com3; ?>" onclick="addcommentimmofi(this.id,<?php echo $numerodecomptefin[$i]; ?>,<?php echo $com;?>)" width="20" height="20"/>
				</td>
				<td width="100px"><input type="button" id="btn<?php echo $com3;?>" onclick="addRenvoieimmofi(this.id, <?php echo $com3;?>)" value="ok" /></td>
			</tr>
			
			<div id="id_renvoie_immofi" style="position:absolute;top:180px;left:392px;background-color:#ccc;width:300px;height:100px;display:none">
