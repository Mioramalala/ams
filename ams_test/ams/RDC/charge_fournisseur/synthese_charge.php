...
function disableButtons() {
	document.getElementById('id_enregistrer_charge').disabled      = true;
	document.getElementById('bt_valider_synthese_charge').disabled = true;
...
$(function(){
		...
		getValidationSynthese('charge', <?php echo $mission_id;?>, function(e) {
			if(e === 'validee') {
				disableButtons();
			}
		});
	...
	/*	ligne = document.getElementById('nbLignes').value;
		for (var i = 1; i <= ligne; i++){
			...
			$.ajax({
				type:'GET',
				url:'RDC/charge_fournisseur/tableau_charge.php',
				data:datatransfert,
					success:function(e){
	...
	$(function(){
		$('#id_retour_charge').click(function(){
			waiting();$('#contenue').load('RDC/charge_fournisseur/feuille_maitresse.php?mission_id='+<?php echo $mission_id; ?>,stopWaiting);		
		});
		
		$('#bt_validation_charge').click(function(){
			$('#dv_synthese_charge').fadeIn();
			getSynthese('charge', <?php echo $mission_id;?>, function(e) {
	...
	function valider_synthese_charge(){
			...
			validerSynthese('charge', <?php echo $mission_id;?>, synthese_charge, function(e) {
				$('#dv_synthese_charge').hide();
				alert('Synth\350ses bien enregistr\351es');
				disableButtons();
			});
...
<body>
	...
	<table style="text-align:center;background-color:#00698d;color:white;width:100%;padding-top:5px;">
		<tr>
			<td><label>SYNTHESE CHARGES-FOURNISSEURS</label></td>
		</tr>
	</table>
	

		<div id="bt_milahatra">
				<input type="button" value="Retour" id="id_retour_charge" style="width:100px;height:30px;float:right"/><br/><br/>
				<input type="button" value="Enregistrer" id="id_enregistrer_charge" onclick="enregistrer_charge()"  style="width:100px;height:30px;float:right"/><br/><br/>
				<input type="button" value="Validation" id="bt_validation_charge" style="width:100px;height:30px;float:right"><br/>
		 ...
		 <div class="cadre">
			<table id="tabB2">
				...
				<?php
					...
					$reponse=$bdd->query('select * FROM tab_synthese_feuille_rdc where SYNTHESE_CYCLE="charge" AND MISSION_ID='.$mission_id);
					(int)$ligne=1;
		...
		<div id="dv_synthese_charge" data-options="handle:'#dragg_c'" align="center" style="display:none;position:absolute;top:150px;left:350px;width:500px;height:395px;background-color:#05abe1;border:5px solid #ccc;">
			<div id="dragg_c" class="td_Titre_Question"><br />SYNTHESE</div>
				<table width="500" bgcolor="#00698d">
					<tr>
						<td class="td_Titre_Question" align="center"><textarea id="txt_synthese_charge" cols="40" style=" width:400px;height:300px"></textarea></td>
					</tr>
					<tr>
						<td align="center">
