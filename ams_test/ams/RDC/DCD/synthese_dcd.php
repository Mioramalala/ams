...
function disableButtons() {
	document.getElementById('id_enregistrer_dcd').disabled      = true;
	document.getElementById('bt_valider_synthese_dcd').disabled = true;
	...
	$(function(){
		...
		getValidationSynthese('dcd', <?php echo $mission_id;?>, function(e) {
			if(e === 'validee') {
				disableButtons();
			}
		});
	...
	function enregistrer_dcd() {
		...
		for (var i = 1; i <= ligne; i++){
			...
			$.ajax({
				type    : 'GET',
				url     : 'RDC/DCD/tableau_dcd.php',
				data    : datatransfert,
				success : function(e) {
					alert('Synthèses bien enregistrées');
					//$('#dv_synthese_dcd').hide();
				}
			});
		//}
	...
	$(function(){
		$('#id_retour_dcd').click(function() {
			waiting();$('#contenue').load('RDC/DCD/feuille_maitresse.php?mission_id='+<?php echo $mission_id; ?>,stopWaiting);		
		});
		
		$('#bt_validation_dcd').click(function() {
			$('#dv_synthese_dcd').fadeIn();
			getSynthese('dcd', <?php echo $mission_id;?>, function(e) {
				document.getElementById("txt_synthese_dcd").value = e;
			});
		});
				
		$("#id_annuler_synthese_dcd").click(function() {
			$('#dv_synthese_dcd').hide();
		});
		
		$('#fermeture_synthese_dcd').click(function() {
			$('#dv_synthese_dcd').hide();
		});
	});
	
	function valider_synthese_dcd(){
		var synthese_dcd = document.getElementById('txt_synthese_dcd').value;

		validerSynthese('dcd', <?php echo $mission_id;?>, synthese_dcd, function(e) {
			$('#dv_synthese_dcd').hide();
			alert('Synth\350ses bien enregistr\351es');
			disableButtons();
		});
...
<form method="post"  id="frmenvoyersynthese">
	<table style="text-align:center;background-color:#00698d;color:white;width:100%;padding-top:5px;">
		<tr>
			<td><label>SYNTHESE DEBITEURS ET CREDITEURS DIVERS</label></td>
		</tr>
	</table>
	
		<div id="bt_milahatra">
				<input type="button" value="Retour" id="id_retour_dcd" style="width:100px;height:30px;float:right"/><br/><br/>
				<input type="button" value="Enregistrer" id="id_enregistrer_dcd" onclick="enregistrer_dcd()"  style="width:100px;height:30px;float:right"/><br/><br/>
				<input type="button" value="Validation" id="bt_validation_dcd" style="width:100px;height:30px;float:right"><br/>
		 ...
		 <div class="cadre">
			<table id="tabB2">
				...
				<?php
					...
					$reponse=$bdd->query('select * FROM tab_synthese_feuille_rdc where SYNTHESE_CYCLE="dcd" AND MISSION_ID='.$mission_id);
					(int)$ligne=1;
...
<!------------------------------------------------------------synthese----------------------------------->
		<div id="dv_synthese_dcd" data-options="handle:'#dragg_c'" align="center" style="display:none;position:absolute;top:150px;left:350px;width:500px;height:395px;background-color:#05abe1;border:5px solid #ccc;">
			<div id="dragg_c" class="td_Titre_Question"><br />SYNTHESE</div>
				<table width="500" bgcolor="#00698d">
					<tr>
						<td class="td_Titre_Question" align="center"><textarea id="txt_synthese_dcd" cols="40" style=" width:400px;height:300px"></textarea></td>
					</tr>
					<tr>
						<td align="center">
							<input type="button" id="bt_valider_synthese_dcd" value="Enregistrer" class="bouton" onclick="valider_synthese_dcd();"/>&nbsp;&nbsp;&nbsp;
							<input type="button" value="Annuler" class="bouton" id="id_annuler_synthese_dcd"/>
						</td>
					</tr>
				</table>
				<div id="fermeture_synthese_dcd" style="top:-20px;left:490px;position:absolute;"><img src="../cycle_achat_image/Fermer.png" /></div>
			</div>
		</div>
