...
function disableButtons() {
	document.getElementById('id_enregistrer_fond').disabled      = true;
	document.getElementById('bt_valider_synthese_fond').disabled = true;
...
$(function(){
		...
		getValidationSynthese('fond', <?php echo $mission_id;?>, function(e) {
			if(e === 'validee') {
				disableButtons();
			}
		});
	...
	function delete_Input(obs, risque, recom, mission_id){
		$.ajax({
			type:'POST',
			url:'RDC/fond_propre/delete_Input_fond.php',
	...
	function enregistrer_fond(){
		datatransfert=$("#frmenvoyersynthese").serialize();
			...
			$.ajax({
				type:'GET',
				url:'RDC/fond_propre/tableau_fond.php',
				data:datatransfert,
					success:function(e){
	...
	$(function(){
		$('#id_retour_fond').click(function(){
			waiting();$('#contenue').load('RDC/fond_propre/feuille_maitresse.php?mission_id='+<?php echo $mission_id; ?>,stopWaiting);		
		});
		
		$('#bt_validation_fond').click(function(){
			$('#dv_synthese_fond').fadeIn();
			getSynthese('fond', <?php echo $mission_id;?>, function(e) {
	...
	function valider_synthese_fond(){
			...
			validerSynthese('fond', <?php echo $mission_id;?>, synthese_fond, function(e) {
				$('#dv_synthese_fond').hide();
				alert('Synth\350ses bien enregistr\351es');
				disableButtons();
			});
...
<form method="post"  id="frmenvoyersynthese">
	<table style="text-align:center;background-color:#00698d;color:white;width:100%;padding-top:5px;">
		<tr>
			<td><label>SYNTHESE FONDS PROPRES</label></td>
		</tr>
	</table>
	
		<div id="bt_milahatra">
				<input type="button" value="Retour" id="id_retour_fond" style="width:100px;height:30px;float:right"/><br/><br/>
				<input type="button" value="Enregistrer" id="id_enregistrer_fond" onclick="enregistrer_fond()"  style="width:100px;height:30px;float:right"/><br/><br/>
				<input type="button" value="Validation" id="bt_validation_fond" style="width:100px;height:30px;float:right"><br/>
		 ...
		 <div class="cadre">
			<table id="tabB2">
				...
				<?php
					...
					$reponse=$bdd->query('select * FROM tab_synthese_feuille_rdc where SYNTHESE_CYCLE="fond" AND  MISSION_ID='.$mission_id);
					(int)$ligne=1;
		...
		<div id="dv_synthese_fond" data-options="handle:'#dragg_c'" align="center" style="display:none;position:absolute;top:150px;left:350px;width:500px;height:395px;background-color:#05abe1;border:5px solid #ccc;">
			<div id="dragg_c" class="td_Titre_Question"><br />SYNTHESE</div>
				<table width="500" bgcolor="#00698d">
					<tr>
						<td class="td_Titre_Question" align="center"><textarea id="txt_synthese_fond" cols="40" style=" width:400px;height:300px"></textarea></td>
					</tr>
					<tr>
						<td align="center">
							<input type="button" id="bt_valider_synthese_fond" value="Enregistrer" class="bouton" onclick="valider_synthese_fond();"/>&nbsp;&nbsp;&nbsp;
