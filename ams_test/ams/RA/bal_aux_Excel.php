			...
			<option value = "N">N
				<?php
					include '../connexion.php';
						$reponse=$bdd->query('SELECT BAL_AUX_CHOIX_ANNEE FROM tab_bal_aux WHERE BAL_AUX_CHOIX_ANNEE="N" AND MISSION_ID='.$mission_id);
							$res=$reponse->fetch();
								...
								if($data1!=" " && $data1=="N"){ 
									...
									var annee= $('#annee').val();
										if(annee='N'){
											alert('Attention fichier balance auxiliare existe déjà.\n Supprimer ancien et enregistrer nouveau!');
											//$('#nomfichier').html('Attention fichier balance générale existe déjà!');
