	...
	<html>
		...
		Exercice:<select id = "annee" onchange="choix_annee()">
			<option value = "N">N
				<?php
					include '../connexion.php';
						$reponse=$bdd->query('SELECT GL_GEN_CHOIX_ANNEE FROM tab_importer WHERE GL_GEN_CHOIX_ANNEE="N" AND MISSION_ID='.$mission_id);
							$res=$reponse->fetch();
								$data1=$res['GL_GEN_CHOIX_ANNEE'];
								if($data1!=" " && $data1=="N"){ 
									echo("<script>
									var annee= $('#annee').val();
										if(annee='N'){
			...
			<option value = "N-1">N-1
				<?php
					include '../connexion.php';
						$reponse2=$bdd->query('SELECT GL_GEN_CHOIX_ANNEE FROM tab_importer WHERE GL_GEN_CHOIX_ANNEE="N-1" AND MISSION_ID='.$mission_id);
							$res2=$reponse2->fetch();
								$data2=$res2['GL_GEN_CHOIX_ANNEE'];
								if($data2!=" " && $data2=="N-1"){ 
									echo("<script>
