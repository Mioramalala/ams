...
if (is_uploaded_file($_FILES['file_gl_auxC5']['tmp_name']))
{ 
	$NameFileko=$_FILES['file_gl_auxC5']['name'];
	//echo $NameFileko;
?>	
...
<?php	
	...
	$result= move_uploaded_file($_FILES['file_gl_auxC5']['tmp_name'],$loader);
	if ($result != 1)
   { 
					...
					fileName=document.getElementById("fileName").value;
						...
						$.ajax({
							...
							success:function(e){
								...
								$.ajax({
									type:'POST',
									url:'enregistrement_gl_auxC5.php',
			...
			<option value = "N">N
				<?php
					include '../connexion.php';
						$reponse=$bdd->query('SELECT GL_AUXC5_CHOIX_ANNEE FROM tab_gl_auxc5 WHERE GL_AUXC5_CHOIX_ANNEE="N" AND MISSION_ID='.$mission_id);
							$res=$reponse->fetch();
								$data1=$res['GL_AUXC5_CHOIX_ANNEE'];
								if($data1!=" " && $data1=="N"){ 
									echo("<script>
									var annee= $('#annee').val();
										if(annee='N'){
