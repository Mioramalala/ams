...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$.ajax({
		    	type : 'POST',
		    	url : "RA/requetTabIncidenceRSCI.php?mission_id="+<?php echo $mission_id; ?>,
		    	dataType: 'json',
		    	data : {cycle:'A'},
...
<?php
	...
	for($i=0; $i<7; $i++){
		$req = $bdd->query('SELECT * FROM tab_synthese_rsci_ra WHERE DOMAINE = "'.utf8_decode($domaines[$i]).'" AND MISSION_ID ='.$mission_id);
		$row = $req->rowCount();
		if($row !=0 ){
			$donnees = $req->fetch();
			...
			echo'
			<tr>
				<td class="td-titre">'.utf8_encode($donnees['DOMAINE']).'</td>
