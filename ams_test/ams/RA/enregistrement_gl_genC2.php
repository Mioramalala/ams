 <?php
	...
	@session_start();
		...
		$compt1[$i]=substr($tableCpt1[$i],0,3);
			...
			//global $entrepriseId;
		//print("ID/prise".$entrepriseId);
		// print ("ici1");
		//exit();
		if(($compt1[$i]>=100 && $compt1[$i]<=159)||($compt1[$i]>=170 && $compt1[$i]<= 179)||($compt1[$i]>=180 && $compt1[$i]<=199)){
			//global $entrepriseId;
		$sqlA = "INSERT INTO tab_gl_genc2 
		(GL_GENC2_COMPTES,GL_GENC2_DATE,GL_GENC2_JL,GL_GENC2_REF,
			GL_GENC2_LIBELLE,GL_GENC2_LEFT,GL_GENC2_DEBIT,GL_GENC2_CREDIT,
			GL_GENC2_SOLDE,GL_GENC2_CYCLE,MISSION_ID,ENTREPRISE_ID,
			GL_GENC2_CHOIX_ANNEE,GL_GENC2_DOCUMENT,UTIL_ID)
 		VALUES ('$tableCpt1[$i]','$tableCpt2[$i]','$tableCpt3[$i]','$tableCpt4[$i]','$tableCpt5[$i]','$tableCpt6[$i]',
 		'$tableCpt7[$i]','$tableCpt8[$i]','$tableCpt9[$i]','A- Fonds propres','$mission_id','$entrepriseId','$annee','$fileName','$UTIL_ID')";
		$bdd->query($sqlA);
