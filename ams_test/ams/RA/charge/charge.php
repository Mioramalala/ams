...
<html>
	<head>
		...
		$(function(){	
			$("#bt_synthese_charge").bind("click", {id : "#bt_synthese_charge"}, checkRole  );
			$("#bt_conclusion_charge").bind("click", {id : "#bt_conclusion_charge"}, checkRole  );
								...
								function checkRole(event){
									...
									var data = JSON.parse(data.responseText);
										if( 
											( (data.admin || data.superviseur || data.tache) && ( event.data.id == "#bt_synthese_charge" || event.data.id == "#enregistrer_page" ) ) ||
											( (data.admin || data.superviseur) && event.data.id == "#bt_conclusion_charge" ) 
										 ){
