...
//////////////Check Role by Sitraka /////////////////
		$(function(){	
			$("#bt_synthese_emprunt").bind("click", {id : "#bt_synthese_emprunt"}, checkRole  );
			$("#bt_conclusion_emprunt").bind("click", {id : "#bt_conclusion_emprunt"}, checkRole  );
								...
								function checkRole(event){
									...
									var data = JSON.parse(data.responseText);
										if( 
											( (data.admin || data.superviseur || data.tache) && ( event.data.id == "#bt_synthese_emprunt" || event.data.id == "#enregistrer_page" ) ) ||
											( (data.admin || data.superviseur) && event.data.id == "#bt_conclusion_emprunt" ) 
										 ){
...
/////////////////End Check Role/////////////////
		...
		{
			<?php
			if(conclusion_existante("emprunt", $mission_id)==true)
			{
			?>
			disable_boutons();
			<?php
		...
		function disable_boutons(){
			//$("#bt_synthese_emprunt").attr("disabled", "disabled");
			//$("#bt_conclusion_emprunt").attr("disabled", "disabled");
			$("#enregistrer_page").attr("disabled", "disabled");
			$( "#bt_synthese_emprunt" ).addClass( "disabled" );
			$( "#bt_conclusion_emprunt" ).addClass( "disabled" );
			$( "#enregistrer_page" ).addClass( "disabled" );

			$("textarea").attr("disabled", "disabled");
			$("#bt_valider_conclusion_emprunt").attr("disabled", "disabled");
			$("#bt_valider_synthese_emprunt").attr("disabled", "disabled");
		}
		$(function(){
				...
				$('.form_upload').submit(function ( e ) {
					...
					var fileInput = $(this).children(":first").toggleClass("file_Import_emprunt");
					var file = fileInput;
					formData.append('file_Import_emprunt', file);

					var xhr = new XMLHttpRequest();
					xhr.open('POST', this.getAttribute('action'), true);
					xhr.onreadystatechange = function () {
				        if(xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
					            $('#contenue').load('../RA/emprunt/emprunt.php?mission_id='+<?php echo $mission_id; ?>, function(){
									stopWaiting();
								});	
				        }
				    };
				...
				$('#bt_synthese_emprunt').click(function(){
					...
					$.ajax({
						...
						success:function(e){
							if(e.trim()!="") document.getElementById("txt_synthese_emprunt").value=e;
							else $("#txt_synthese_emprunt").attr("placeholder", "Redigez votre synthese ici");
						}
					});
				});
				...
				$('#bt_conclusion_emprunt').click(function(){
					...
					$.ajax({
						...
						data:{mission_id:mission_id},
						success:function(e){
							if(e.trim()!="") document.getElementById("txt_conclusion_emprunt").value=e;
							else $("#txt_conclusion_emprunt").attr("placeholder", "Redigez votre conclusion ici");
						}
					});
				});
		...
		function valider_synthese_emprunt(){
			...
			$.ajax({
				...
				data:{synthese_emprunt:synthese_emprunt,mission_id:mission_id},
					success:function(e){
						...
						$('#dv_synthese_emprunt').hide();
							$("#bt_synthese_emprunt").attr("class","disabled");
							$("#bt_synthese_emprunt").attr("disabled");
		 ...
		 function conclusion_empruntdette(){
		...
		function addcommentemprunt(i){
					...
					var enregistrer_tableau_import9=$('#commentemprunt'+i).val();
					var mission_id=<?php echo $mission_id;?>
		...
		function addRenvoieemprunt(id,compte){
			$('#id_renvoie_emprunt'+compte).show();
		}
							...
							data:transferdata,
								success:function(e){
									 $("#enregistrer_page").attr("class","disabled");
										...
										$("#bt_conclusion_emprunt").removeClass("disabled");
										$("#bt_conclusion_emprunt").removeAttr("disabled");
		...
		function fileSubmit(num){
			$('#id_renvoie_emprunt'+num).hide();
			if($("#emprunt_form_upload"+num+" > input").get(0).files.length !=0 ){
				$("#emprunt_form_upload"+num).submit();
			}
		}
		</script>
	</head>
	...
	?>
		<table width="100%"  style="text-align:center;">
			<tr>
				<td height="50" bgcolor="#00698d"><center><label class="grand_Titre" style="color:white;">VERIFICATION DE LA VARIATION DES EMPRUNTS ET DETTES FINANCIERES</label></center></td>
			</tr>
			<tr>
       ...
       <div id="bt_milahatra" style="position:relative ; top:-40px">
			...
			<input type="button" <?php if(conclusion_existante("emprunt", $mission_id)==true){echo " class='disabled' ";}?> value="Synth&egrave;se" id="bt_synthese_emprunt" style="width:80">
			<input type="button"  class='disabled' name="btconclusion" value="Conclusion" id="bt_conclusion_emprunt" style="width:80"/>
			<input type ="button" <?php if(conclusion_existante("emprunt", $mission_id)==true){echo "disabled class='disabled' ";}?> value = "Enregistrer" id="enregistrer_page" onclick="enregistrer_tableau_emprunt();" style="width:80;"/>
	   </div>
	...
	<form id="frm_Revuecyletab" method="post">
		<table class="table_parcycle" width="1200px">
			<?php
				...
				// WHERE E2.IMPORT_CYCLE='J- Emprunts et dettes financi&egrave;res' AND E2.MISSION_ID=".$mission_id." AND E1.IMPORT_CHOIX_ANNEE='N-1' AND E2.IMPORT_CHOIX_ANNEE='N' GROUP BY E2.IMPORT_COMPTES");
				
//				$reponse=$bdd->query("SELECT  DISTINCT E1.IMPORT_COMPTES,E2.IMPORT_COMPTES,E2.IMPORT_ID,E2.IMPORT_INTITULES,E2.IMPORT_DEBIT,E2.IMPORT_CREDIT,E1.IMPORT_SOLDE as soldeN1,E2.IMPORT_SOLDE as soldeN,E1.IMPORT_CHOIX_ANNEE,E2.IMPORT_CHOIX_ANNEE,(E2.IMPORT_SOLDE-E1.IMPORT_SOLDE)as Valeur FROM tab_importer E1, tab_importer E2
//				WHERE E2.IMPORT_CYCLE='J- Emprunts et dettes financi&egrave;res' AND E2.MISSION_ID=".$mission_id." AND E1.IMPORT_CHOIX_ANNEE='N-1' AND E2.IMPORT_CHOIX_ANNEE='N' GROUP BY E2.IMPORT_COMPTES");
				
				$reponse=remplir_tab_par_cycle('J- Emprunts et dettes financi&egrave;res',$mission_id);
			...
			<tr height="70" id="idBalGen<?php echo $com1;?>" onclick="makaid(<?php echo $donnees['IMPORT_ID'];?>)" value="<?php echo $donnees['IMPORT_ID'];?>">
				...
				<td width="200px">
					<textarea cols="5" name="commentaire[]" rows="4" <?php // if(conclusion_existante("emprunt", $mission_id)==true){echo "disabled class='disabled' ";}?> id="commentemprunt<?php echo $com1?>"><?php echo afficherCommentaire($import_compteN, 'emprunt', $mission_id)?></textarea>
					
					<img src="../RA/image/add.jpg" style="cursor:pointer;" id="com1<?php echo $com1; ?>" onclick="addcommentemprunt(<?php echo $com1;?>)" width="20" height="20"/>
				</td>
				<td width="70px">
					<?php
						...
						$renvoielien="";
						$reponse2=$bdd->query('select RENVOIE_LIEN from tab_renvoie where MISSION_ID='.$mission_id.' AND RENVOIE_OBJECTIF="emprunt" AND IMPORT_ID='.$numerodecompte[$i]);
					...
					<img src="RA/image/up.png" alt="upload" style="cursor:pointer;" id="btn<?php echo $com1;?>" onclick="addRenvoieemprunt(this.id, <?php echo $com1;?>)"/></td>
			</tr>
			<td>
			<!-- first form in table is deleted so I created this -->
			<form class="created">
			</form>
			<div id="id_renvoie_emprunt<?php echo $com1;?>" style="position:absolute;top:180px;left:392px;background-color:#ccc;width:300px;height:100px;display:none">
				<form id="emprunt_form_upload<?php echo $com1;?>" class="form_upload" method = "post" enctype="multipart/form-data" action = "RA/emprunt/importemprunt.php?mission_id=<?php echo $mission_id;?>&importid=<?php echo $numerodecompte[$i]; ?>">
					<input type = "file" required class = "file_Import_emprunt" name = "file_Import_emprunt"/>
				...
				<div class="fermeture_import_emprunt" style="top:-18px;left:280px;position:absolute;"><img src="../cycle_achat_image/Fermer.png" /></div>
			</div> 
			</td>
			<?php
				}
...
<div id="centre_emprunt" align="center">
		...
		<table width="500" bgcolor="#00698d">
				<td class="td_Titre_Question" align="center"><textarea id="txt_synthese_emprunt" cols="40" rows="20"  ></textarea>
				</td>
			</tr>
			<tr>
				<td align="center">
					<input type="button" id="bt_valider_synthese_emprunt" value="Enregistrer" class="bouton" onclick="valider_synthese_emprunt();" <?php //if($conclusionimmo!=0) echo 'disabled'; ?> />&nbsp;&nbsp;&nbsp;
		...
		<div id="dv_conclusion_emprunt" data-options="handle:'#dragg_c'" align="center" style="display:none;position:absolute;top:150px;left:350px;width:500px;height:395px;background-color:#05abe1;border:5px solid #ccc;">
			...
			<table width="500" bgcolor="#00698d">
					<td class="td_Titre_Question" align="center"><textarea id="txt_conclusion_emprunt" cols="40" rows="20" ></textarea>
					</td>
				</tr>
				<tr>
					<td align="center">
						<input type="button" id="bt_valider_conclusion_emprunt" value="Valider" class="bouton" onclick="conclusion_empruntdette();">&nbsp;&nbsp;&nbsp;
