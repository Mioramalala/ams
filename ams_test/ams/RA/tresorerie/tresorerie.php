<?php
	...
	$reponse2=$bdd->query('SELECT count(IMPORT_ID) AS COMPTE FROM tab_importer where IMPORT_CYCLE ="E- Tr&eacute;soreries"');
...
//////////////Check Role by Sitraka /////////////////
		$(function(){	
			$("#bt_synthese_tres").bind("click", {id : "#bt_synthese_tres"}, checkRole  );
			$("#bt_conclusion_tres").bind("click", {id : "#bt_conclusion_tres"}, checkRole  );
								...
								function checkRole(event){
									...
									var data = JSON.parse(data.responseText);
										if( 
											( (data.admin || data.superviseur || data.tache) && ( event.data.id == "#bt_synthese_tres" || event.data.id == "#enregistrer_page" ) ) ||
											( (data.admin || data.superviseur) && event.data.id == "#bt_conclusion_tres" ) 
										 ){
...
/////////////////End Check Role/////////////////
		...
		{
			<?php
			if(conclusion_existante("tresorerie", $mission_id)==true)
			{
			?>
			disable_boutons();
			<?php
		...
		function disable_boutons(){
			//$("#bt_synthese_tres").attr("disabled", "disabled");
			//$("#bt_conclusion_tre").attr("disabled", "disabled");
			$("#enregistrer_page").attr("disabled", "disabled");
			$( "#bt_synthese_tres" ).addClass( "disabled" );
			$( "#bt_conclusion_tres" ).addClass( "disabled" );
			$( "#enregistrer_page" ).addClass( "disabled" );

			$("textarea").attr("disabled", "disabled");
			$("#bt_valider_conclusion_tres").attr("disabled", "disabled");
			$("#bt_valider_synthese_tres").attr("disabled", "disabled");
		...
		$(function(){
				...
				$('.form_upload').submit(function ( e ) {
					...
					var fileInput = $(this).children(":first").toggleClass("file_Import_tres");
					var file = fileInput;
					formData.append('file_Import_tres', file);

					var xhr = new XMLHttpRequest();
					xhr.open('POST', this.getAttribute('action'), true);
					xhr.onreadystatechange = function () {
				        if(xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
					        $('#contenue').load('../RA/tresorerie/tresorerie.php?mission_id='+<?php echo $mission_id; ?>, function(){
								stopWaiting();
							});	
				        }
				    };
				...
				$('#bt_synthese_tres').click(function()
				{
					...
					$.ajax({
						...
						success:function(e){
							if(e.trim()!="") document.getElementById("txt_synthese_tres").value=e;
							else $("#txt_synthese_tres").attr("placeholder", "Redigez votre synthese ici");	
						}
					});
				});
		...
		$(function(){
				$('#bt_conclusion_tres').click(function(){
					...
					$.ajax({
						...
						data:{mission_id:mission_id},
						success:function(e){
							if(e.trim()!="") document.getElementById("txt_conclusion_tres").value=e;
							else $("#txt_conclusion_tres").attr("placeholder", "Redigez votre conclusion ici");
						}
					});
				});
			...
			$.ajax({
				...
				data:{synthese_tres:synthese_tres,mission_id:mission_id},
					success:function(e)
					{
						...
						$("#bt_synthese_tres").attr("class","disabled");
						$("#bt_synthese_tres").attr("disabled");
		...
		function addcommenttres(i){
					...
					var enregistrer_tableau_import9=$('#commenttres'+i).val();
					var mission_id=<?php echo $mission_id;?>;
		...
		function enregistrer_tableau_tres()
		{
					...
					$.ajax({
							...
							success:function(e)
							{
									...
									$("#bt_conclusion_tres").removeClass("disabled");
									$("#bt_conclusion_tres").removeAttr("disabled");
		...
		function fileSubmit(num){
			$('#id_renvoie_tres'+num).hide();
			if($("#tres_form_upload"+num+" > input").get(0).files.length !=0 ){
				$("#tres_form_upload"+num).submit();
			}
		}
		</script>
	</head>
	...
	<table width="100%"  style="text-align:center;">
			<tr>
				<td height="50" bgcolor="#00698d"><center><label class="grand_Titre" style="color:white;">VERIFICATION DE LA VARIATION DE TRESORERIE</label></center></td>
			</tr>
			<tr>
	...
	<div id="bt_milahatra" style="position:relative ; top:-40px">
			...
			<input type="button" <?php if(conclusion_existante("tresorerie", $mission_id)==true){echo " class='disabled' ";}?>  value="Synth&egrave;se" id="bt_synthese_tres" style="width:80">
			<input type="button" <?php ?>  class='disabled' name="btconclusion" value="Conclusion" id = "bt_conclusion_tres" style="width:80"/>
			<input type ="button" <?php if(conclusion_existante("tresorerie", $mission_id)==true){echo "disabled class='disabled' ";}?> value = "Enregistrer" id="enregistrer_page" onclick="enregistrer_tableau_tres();" style="width:80;"/>
	</div>
...
//				WHERE E2.IMPORT_CYCLE='E- Tr&eacute;soreries' AND E2.MISSION_ID=".$mission_id." AND E1.IMPORT_CHOIX_ANNEE='N-1' AND E2.IMPORT_CHOIX_ANNEE='N' GROUP BY E2.IMPORT_COMPTES");
				
				$reponse=remplir_tab_par_cycle('E- Tr&eacute;soreries',$mission_id);
			...
			<tr height="70" id="idBalGen<?php echo $com1;?>" onclick="makaid(<?php echo $donnees['IMPORT_ID'];?>)" value="<?php echo $donnees['IMPORT_ID'];?>">
				...
				<input type="hidden" hp if(empty($import_variation)){}else{echo number_format($import_variation, 2, '.', ' ');}?></td>
				...
				<td width="200px">
					<textarea  name="commentaire[]"  cols="5" <?php //if(conclusion_existante("tresorerie", $mission_id)==true) {echo "disabled class='disabled' ";}?> rows="4" id="commenttres<?php echo $com1?>"><?php echo afficherCommentaire($import_compteN, 'tresorerie', $mission_id)?></textarea>
					
					<img src="../RA/image/add.jpg" style="cursor:pointer;" id="com1<?php echo $com1; ?>" onclick="addcommenttres(<?php echo $com1;?>)" width="20" height="20"/>
				</td>
				<td width="70px">
					<?php
						...
						$renvoielien="";
						$reponse2=$bdd->query('select RENVOIE_LIEN from tab_renvoie where MISSION_ID='.$mission_id.' AND RENVOIE_OBJECTIF="tresorerie" AND IMPORT_ID='.$donnees['IMPORT_ID']);
					...
					<img src="RA/image/up.png" alt="upload" style="cursor:pointer;" id="btn<?php echo $com1;?>" onclick="addRenvoietres(this.id, <?php echo $com1;?>)"/></td>
			</tr>
			<td>
			<!-- first form in table is deleted so I created this -->
			<form class="created">
			</form>
			<div class="modal_upload" id="id_renvoie_tres<?php echo $com1; ?>" style="position:absolute;top:180px;left:392px;background-color:#ccc;width:300px;height:100px;display:none">
				<form id="tres_form_upload<?php echo $com1;?>" class="form_upload" method = "post" enctype="multipart/form-data" action = "RA/tresorerie/importtres.php?mission_id=<?php echo $mission_id;?>&importid=<?php echo $donnees['IMPORT_ID']; ?>">
					<input type = "file" required class= "file_Import_tres" name = "file_Import_tres"/>
				...
				<div class="fermeture_import_tres" style="top:-18px;left:280px;position:absolute;"><img src="../cycle_achat_image/Fermer.png" /></div>
			</div> 
			</td>
			<?php
				}
...
<div id="centre_tres" align="center">
		...
		<table width="500" bgcolor="#00698d">
				<td class="td_Titre_Question" align="center"><textarea id="txt_synthese_tres" cols="40" rows="20"></textarea>
				</td>
			</tr>
			<tr>
				<td align="center">
					<input type="button" id="bt_valider_synthese_tres" value="Enregistrer" class="bouton" onclick="valider_synthese_tres();" <?php //if($conclusionimmo!=0) echo 'disabled'; ?> />&nbsp;&nbsp;&nbsp;
		...
		<div id="dv_conclusion_tres" data-options="handle:'#dragg_c'" align="center" style="display:none;position:absolute;top:150px;left:350px;width:500px;height:395px;background-color:#05abe1;border:5px solid #ccc;">
			...
			<table width="500" bgcolor="#00698d">
					<td class="td_Titre_Question" align="center"><textarea id="txt_conclusion_tres" cols="40" rows="20"></textarea>
					</td>
				</tr>
				<tr>
					<td align="center">
						<input type="button" id="bt_valider_conclusion_tres" value="Valider" class="bouton" onclick="conclusion_tresorerie();">&nbsp;&nbsp;&nbsp;
