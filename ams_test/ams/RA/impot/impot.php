...
//////////////Check Role by Sitraka /////////////////
		$(function(){	
			$("#bt_synthese_impot").bind("click", {id : "#bt_synthese_impot"}, checkRole  );
			$("#bt_conclusion_impot").bind("click", {id : "#bt_conclusion_impot"}, checkRole  );
								...
								function checkRole(event){
									...
									var data = JSON.parse(data.responseText);
										if( 
											( (data.admin || data.superviseur || data.tache) && ( event.data.id == "#bt_synthese_impot" || event.data.id == "#enregistrer_page" ) ) ||
											( (data.admin || data.superviseur) && event.data.id == "#bt_conclusion_impot" ) 
										 ){
...
/////////////////End Check Role/////////////////
		...
		{
			<?php
			if(conclusion_existante("impot", $mission_id)==true)
			{
			?>
			disable_boutons();
			<?php
		...
		{
			//$("#bt_synthese_impot").attr("disabled", "disabled");
			//$("#bt_conclusion_impot").attr("disabled", "disabled");
			$("#enregistrer_page").attr("disabled", "disabled");
			$( "#bt_synthese_impot" ).addClass( "disabled" );
			$( "#bt_conclusion_impot" ).addClass( "disabled" );
			$( "#enregistrer_page" ).addClass( "disabled" );


			$("textarea").attr("disabled", "disabled");
			$("#bt_valider_conclusion_impot").attr("disabled", "disabled");
			$("#bt_valider_synthese_impot").attr("disabled", "disabled");
		}
		$(function()
		{
				...
				$('.form_upload').submit(function ( e ) {
					...
					var fileInput = $(this).children(":first").toggleClass("file_Import_impot");
					var file = fileInput;
					console.log(file);
					formData.append('file_Import_impot', file);

					var xhr = new XMLHttpRequest();
					xhr.open('POST', this.getAttribute('action'), true);
					xhr.onreadystatechange = function () {
				        if(xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
					          $('#contenue').load('../RA/impot/impot.php?mission_id='+<?php echo $mission_id; ?>, function(){
								stopWaiting();
							});		
				        }
				    };
				...
				$('#bt_synthese_impot').click(function(){
					...
					$.ajax({
						...
						success:function(e){
							if(e.trim()!="") document.getElementById("txt_synthese_impot").value=e;
							else $("#txt_synthese_impot").attr("placeholder", "Redigez votre synthese ici");
						}
					});
				});
		...
		$(function(){
				$('#bt_conclusion_impot').click(function(){
					...
					$.ajax({
						...
						data:{mission_id:mission_id},
						success:function(e){
							if(e.trim()!="") document.getElementById("txt_conclusion_impot").value=e;
							else $("#txt_conclusion_impot").attr("placeholder", "Redigez votre conclusion ici");	
						}
					});
				});
		...
		function valider_synthese_impot(){
			...
			$.ajax({
				...
				data:{synthese_impot:synthese_impot,mission_id:mission_id},
					success:function(e){
							$('#dv_synthese_impot').hide();
							$("#bt_synthese_impot").attr("class","disabled");
							$("#bt_synthese_impot").attr("disabled");
		...
		function addcommentimpot(i){
					...
					var enregistrer_tableau_import9=$('#commentimpot'+i).val();
					var mission_id=<?php echo $mission_id;?>;
		...
		function enregistrer_tableau_impot()
		{
					...
					$.ajax({
							...
							success:function(e)
							{
										...
										$("#bt_conclusion_impot").removeClass("disabled");
										$("#bt_conclusion_impot").removeAttr("disabled");
		...
		function fileSubmit(num){
			$('#id_renvoie_impot'+num).hide();
			if($("#impot_form_upload"+num+" > input").get(0).files.length !=0 ){
				$("#impot_form_upload"+num).submit();
			}
		}
		</script>
	</head>
	...
	<?php
		...
		$conclusionimpot=$donnees['CONCLUSION_RA_ID'];	

	?>
	<table width="100%"  style="text-align:center;">
		<tr>
				<td height="50" bgcolor="#00698d"><center><label class="grand_Titre" style="color:white;">VERIFICATION DE LA VARIATION DES IMPOTS ET TAXES</label></center></td>
			</tr>
			<tr>
	...
	<div id="bt_milahatra" style="position:relative ; top:-40px">
			...
			<input type="button" <?php if(conclusion_existante("impot", $mission_id)==true){echo " class='disabled' ";}?>   value="Synth&egrave;se" id="bt_synthese_impot" style="width:80">
			<input type="button" <?php ?>  class='disabled' name="btconclusion" value="Conclusion" id = "bt_conclusion_impot" style="width:80"/>
			<input type ="button" <?php if(conclusion_existante("impot", $mission_id)==true){echo "disabled class='disabled' ";}?> value = "Enregistrer" id="enregistrer_page" onclick="enregistrer_tableau_impot();" style="width:80;"/>
	</div>
	...
	<form id="frm_Revuecyletab" method="post">
		<table class="table_parcycle" width="1200px">
			<?php
				...
				$reponse=remplir_tab_par_cycle('I- Imp&ocirc;ts et taxes',$mission_id);
			...
			<tr height="70" id="idBalGen<?php echo $com1;?>" onclick="makaid(<?php echo $donnees['IMPORT_ID'];?>)" value="<?php echo $donnees['IMPORT_ID'];?>">
				...
				<td width="200px">
					<textarea name="commentaire[]" cols="5" rows="4" <?php //if(conclusion_existante("impot", $mission_id)==true){echo "disabled class='disabled' ";}?> id="commentimpot<?php echo $com1?>"><?php echo afficherCommentaire($import_compteN, 'impot', $mission_id)?></textarea>
					
					<img src="../RA/image/add.jpg" style="cursor:pointer;" id="com1<?php echo $com1; ?>" onclick="addcommentimpot(<?php echo $com1;?>)" width="20" height="20"/>
				</td>
				<td width="70px">
					<?php
						...
						$renvoielien="";
						$reponse2=$bdd->query('select RENVOIE_LIEN from tab_renvoie where MISSION_ID='.$mission_id.' AND RENVOIE_OBJECTIF="impot" AND IMPORT_ID='.$donnees['IMPORT_ID']);
					...
					<img src="RA/image/up.png" alt="upload" style="cursor:pointer;" id="btn<?php echo $com1;?>" onclick="addRenvoieimpot(this.id, <?php echo $com1;?>)"/></td>
			</tr>
			<td>
			<!-- first form in table is deleted so I created this -->
			<form class="created">
			</form>
			<div class="modal_upload" id="id_renvoie_impot<?php echo $com1;?>" style="position:absolute;top:180px;left:392px;background-color:#ccc;width:300px;height:100px;display:none">
				<form id="impot_form_upload<?php echo $com1;?>" class="form_upload" method = "post" enctype="multipart/form-data" action = "RA/impot/importimpot.php?mission_id=<?php echo $mission_id;?>&importid=<?php echo $donnees['IMPORT_ID']; ?>">
					<input type = "file" required class = "file_Import_impot" name = "file_Import_impot"/>
				...
				<div class="fermeture_import_impot" style="top:-18px;left:280px;position:absolute;"><img src="../cycle_achat_image/Fermer.png" /></div>
			</div> 
			</td>
			<?php
				}
...
<div id="centre_impot" align="center">
		...
		<table width="500" bgcolor="#00698d">
				<td class="td_Titre_Question" align="center"><textarea id="txt_synthese_impot" cols="40" rows="20"></textarea>
				</td>
			</tr>
			<tr>
				<td align="center">
		...
		<div id="dv_conclusion_impot" data-options="handle:'#dragg_c'" align="center" style="display:none;position:absolute;top:150px;left:350px;width:500px;height:395px;background-color:#05abe1;border:5px solid #ccc;">
			...
			<table width="500" bgcolor="#00698d">
					<td class="td_Titre_Question" align="center"><textarea id="txt_conclusion_impot" cols="40" rows="20"></textarea>
					</td>
				</tr>
				<tr>
					<td align="center">
						<input type="button" id="bt_valider_conclusion_impot" value="Valider" class="bouton" onclick="conclusion_impottaxe();">&nbsp;&nbsp;&nbsp;
