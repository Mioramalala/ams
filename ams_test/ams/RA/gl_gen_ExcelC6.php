...
if (is_uploaded_file($_FILES['file_gl_genC6']['tmp_name']))
{ 
	$NameFileko=$_FILES['file_gl_genC6']['name'];
	//echo $NameFileko;
?>	
...
<?php	
	...
	set_time_limit(-1);
	$result= move_uploaded_file($_FILES['file_gl_genC6']['tmp_name'],$loader);
	if ($result != 1)
   { 
...
?>
	<html>
					...
					fileName=document.getElementById("fileName").value;
						...
						$.ajax({
							...
							success:function(e){
								...
								$.ajax({
									type:'POST',
									url:'enregistrement_gl_genC6.php',
		...
		<div id="nomfichier"></div>
			...
			<option value = "N">N
				<?php
						$reponse=$bdd->query('SELECT GL_GENC6_CHOIX_ANNEE FROM tab_gl_genc6 WHERE GL_GENC6_CHOIX_ANNEE="N" AND MISSION_ID='.$mission_id);
							$res=$reponse->fetch();
								$data1=$res['GL_GENC6_CHOIX_ANNEE'];
								if($data1!=" " && $data1=="N"){ 
									echo("<script>
									var annee= $('#annee').val();
										if(annee='N'){
	...
	//$bd = new PDO('mysql:host='.$connex.';dbname=tmsconsuams','joomla', 'joomla');


					$req = "SELECT  compt1,compt2 FROM tab_bal_gen_mission WHERE 
					compt1 LIKE '6%' AND id_mission=".$mission_id." ORDER BY compt1 ASC";
					//echo ($req);
			...
			<input type = "button" id = "id_suppr" value="Supprimer Comptes" onclick="supprimer()" style = "float:right;width:150px;height:30px"/><br/><br/>
