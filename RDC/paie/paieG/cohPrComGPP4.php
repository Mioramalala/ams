...
<?php session_start();
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$sql1="SELECT RDC_REPONSE,RDC_COMMENTAIRE FROM tab_rdc WHERE RDC_RANG= 4 AND RDC_OBJECTIF='G' AND MISSION_ID=".$_SESSION['idMission']." AND RDC_CYCLE='paie'";
	$rep=$bdd->query($sql1);
	$don=$rep->fetch();
...
<html>
		...
		<script>
			$(function(){
				$("#renvoiRevAnPP").click(function(){
					//$("#ChampGauche").load("RDC/paie/paieG/TabDG4PP.php");
					$('#ChampDroite').show();
					$('#suivantPP').show();
					$('#precedPP').show();
					$('#retourPP').hide();
				...
				$("#suivantPP").click(function(){
					...
					if((rep1=="non" && cmt1=="") || rep1==""){ 
						...
						$.ajax({
							...
							data:{rep:rep1,cmt:cmt1,rang:4,cycle:'paie',objet:'G'},
							success:function(e){
							//	alert(e);
							}				
						});
	...
	<body>		
		<div id="GranTitre">
			H - JURIDIQUE, FISCAL ET DIVERS : Partie IV
		</div>		
		<div id="ChampGauche">
			
				<p><strong>Travaux à faire :</strong> Examiner les saisies arrêts sur salaires. </p>
				
				<h4>Documents et infos à obtenir</h4>	
				<p>
					<ul>
						<li>Lettre du tribunal.</li>
					</ul>
				</p>
				<h4>Question :</h4>	
				<p>
					<ul>
						<li>Assurez-vous que tous les saisies arrêts ont été excécutés et traités correctement ?</li>
					</ul>
				</p>
				...
				<div id="uploadJustification" style="display:none;">
					...
					<form enctype="multipart/form-data" action="RDC/paie/paieG/envoi.php" method="POST" id="form" target="uploadFrame">
		...
		<div>
			...
			<div id="ChampDroite" 
				...
				?> >
				<h5><u>Question :</u></h5>
				<p>Assurez-vous que tous les saisies arrêts ont été excécutés et traités correctement ?
