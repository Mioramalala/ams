...
<?php session_start();
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$sql1="SELECT RDC_REPONSE,RDC_COMMENTAIRE FROM tab_rdc WHERE RDC_RANG= 2 AND RDC_OBJECTIF='G' AND MISSION_ID=".$_SESSION['idMission']." AND RDC_CYCLE='paie'";
	$rep=$bdd->query($sql1);
	$don=$rep->fetch();
...
<html>
		...
		<script>
			$(function(){
				$("#renvoiRevAnPP").click(function(){
					//$("#ChampGauche").load("RDC/paie/paieG/TabDG2PP.php");
					$('#ChampDroite').show();
					$('#suivantPP').show();
					$('#precedPP').show();
					$('#retourPP').hide();
				...
				$("#suivantPP").click(function(){
					...
					if((rep1=="non" && cmt1=="") || rep1==""){ 
						...
						$.ajax({
							...
							data:{rep:rep1,cmt:cmt1,rang:2,cycle:'paie',objet:'G'},
							success:function(e){
							//	alert(e);
							}				
						});
	...
	<body>		
		...
		<div id="ChampGauche">
				...
				<p><strong>Travaux à faire :</strong> Examiner les rémunérations et avantages alloués aux dirigeants et mandataires. </p>
				
				<h4>Documents et infos à obtenir</h4>	
				<p>
					<ul>
						<li>Déclarations IRSA et grand livres des comptes.</li>
					</ul>
				</p>
				<h4>Question :</h4>	
				<p>
					<ul>
						<li>Assurez-vous que toutes rémunérations et avantages alloués aux dirigeants et mandataires ont été déclarés à l'IRSA ?</li>
					</ul>
				</p>
				...
				<div id="uploadJustification" style="display:none;">
					<div id="uploadJustification">
						...
						<?php 
							$sql = "SELECT nom,extension FROM tab_pieces_justificatives WHERE mission_id=".$_SESSION['idMission']." AND pour = 'paie_G2'";
							$rep =$bdd->query($sql);
							
							if($donnee = $rep->fetch()) {
						?>
							<a href="<?php echo "pieces_justificatives/pj_paie_G2_".$_SESSION['idMission'].'.'.$donnee['extension'] ?>" target="_blank"><?php echo $donnee['nom'] ?></a>
						<?php
							}
						?>
						<form enctype="multipart/form-data" action="RDC/envoi_pieces_justificatives.php" method="POST" id="form" target="uploadFrame">
							...
							<input type="hidden" name="pour" value="paie_G2" />
		...
		<div>
			...
			<div id="ChampDroite" 
				...
				?> >
				<h5><u>Question :</u></h5>
				<p>Assurez-vous que toutes rémunérations et avantages alloués aux dirigeants et mandataires ont été déclarés à l'IRSA ?
