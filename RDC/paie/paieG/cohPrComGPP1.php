<?php session_start();
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$sql1="SELECT RDC_REPONSE,RDC_COMMENTAIRE FROM tab_rdc WHERE RDC_RANG= 1 AND RDC_OBJECTIF='G' AND MISSION_ID=".$_SESSION['idMission']." AND RDC_CYCLE='paie'";
	$rep=$bdd->query($sql1);
	$don=$rep->fetch();
...
<html>
		...
		<script>
			$(function(){
				$("#renvoiRevAnPP").click(function(){
					//$("#ChampGauche").load("RDC/paie/paieG/TabDG1PP.php");
					$('#ChampDroite').show();
					$('#suivantPP').show();
					$('#precedPP').show();
					$('#retourPP').hide();
				...
				$("#suivantPP").click(function(){
					...
					if((rep1=="non" && cmt1=="") || rep1==""){ 
						...
						$.ajax({
							...
							data:{rep:rep1,cmt:cmt1,rang:1,cycle:'paie',objet:'G'},
							success:function(e){
							//	alert(e);
							}				
						});
	...
	<body>		
		...
		<div id="ChampGauche">
				...
				<p><strong>Travaux à faire :</strong> Examiner le traitement fiscal des charges liées au personnel (congés payés, dépenses somptuaires). </p>
				
				<h4>Documents et infos à obtenir</h4>	
				<p>
					<ul>
						<li>Résultat fiscal.</li>
					</ul>
				</p>
				<h4>Question :</h4>	
				<p>
					<ul>
						<li>Confirmez-vous que les dotations aux provisions pour congés payés et autres avantages non déclarés à l'IRSA ont été réintégrés lors de la détermination du résultat fiscal, et les reprises déduites ?</li>
					</ul>
				</p>
				...
				<div id="uploadJustification" style="display:none;">
					<div id="uploadJustification">
						...
						<?php 
							$sql = "SELECT nom,extension FROM tab_pieces_justificatives WHERE mission_id=".$_SESSION['idMission']." AND pour = 'paie_G1'";
							$rep =$bdd->query($sql);
							
							if($donnee = $rep->fetch()) {
						?>
							<a href="<?php echo "pieces_justificatives/pj_paie_G1_".$_SESSION['idMission'].'.'.$donnee['extension'] ?>" target="_blank"><?php echo $donnee['nom'] ?></a>
						<?php
							}
						?>
						<form enctype="multipart/form-data" action="RDC/envoi_pieces_justificatives.php" method="POST" id="form" target="uploadFrame">
							...
							<input type="hidden" name="pour" value="paie_G1" />
