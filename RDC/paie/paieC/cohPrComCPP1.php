...
<?php session_start();
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$sql1="SELECT RDC_REPONSE,RDC_COMMENTAIRE FROM tab_rdc WHERE RDC_RANG = 1 AND RDC_OBJECTIF='C' AND MISSION_ID=".$_SESSION['idMission']." AND RDC_CYCLE='paie'";
	$rep=$bdd->query($sql1);
	$don=$rep->fetch();
...
<html>
		...
		<script>
			$(function(){
				$("#renvoiRevAnPP").click(function(){
					//$("#ChampGauche").load("RDC/paie/paieC/TabDC1PP.php");
					$('#ChampDroite').show();
					$('#suivantPP').show();
					$('#precedPP').show();
					$('#retourPP').hide();
				...
				$("#suivantPP").click(function(){
					...
					if((rep1=="non" && cmt1=="") || rep1==""){ 
						...
						$.ajax({
							...
							data:{rep:rep1,cmt:cmt1,rang:1,cycle:'paie',objet:'C'},
							success:function(e){
							//	alert(e);
							}				
						});
	...
	<body>		
		...
		<div id="ChampGauche">
				...
				<p><strong>Travaux à faire :</strong> Tester l'établissement de la paie. </p>
				...
				<p>
					<ul>
						<li>L'établissement de la paie est-il correct ?</li>
					</ul>
				</p>
				...
				<div id="uploadJustification" style="display:none;">
					...
					<?php 
						$sql = "SELECT nom,extension FROM tab_pieces_justificatives WHERE mission_id=".$_SESSION['idMission']." AND pour = 'paie_personnel_c1'";
						$rep =$bdd->query($sql);
						
						if($donnee = $rep->fetch()) {
					?>
						<a href="<?php echo "pieces_justificatives/pj_paie_personnel_c1_".$_SESSION['idMission'].'.'.$donnee['extension'] ?>" target="_blank"><?php echo $donnee['nom'] ?></a>
					<?php
						}
					?>
					<form enctype="multipart/form-data" action="RDC/envoi_pieces_justificatives.php" method="POST" id="form" target="uploadFrame">
						...
						<input type="hidden" name="pour" value="paie_personnel_c1" />
		...
		<div>
			...
			<div id="ChampDroite" 
				...
				?> >
				<h5><u>Question :</u></h5>
				<p>L'établissement de la paie est-il correct ? 
