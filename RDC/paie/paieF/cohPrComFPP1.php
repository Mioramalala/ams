...
<?php session_start();
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$sql1="SELECT RDC_REPONSE,RDC_COMMENTAIRE FROM tab_rdc WHERE RDC_RANG= 1 AND RDC_OBJECTIF='F' AND MISSION_ID=".$_SESSION['idMission']." AND RDC_CYCLE='paie'";
	$rep=$bdd->query($sql1);
	$don=$rep->fetch();
...
<html>
		...
		<script>
			$(function(){
				$("#renvoiRevAnPP").click(function(){
					//$("#ChampGauche").load("RDC/paie/paieF/TabDF1PP.php");
					$('#ChampDroite').show();
					$('#suivantPP').show();
					$('#precedPP').show();
					$('#retourPP').hide();
				...
				$("#suivantPP").click(function(){
					...
					if((rep1=="non" && cmt1=="") || rep1==""){ 
						...
						$.ajax({
							...
							data:{rep:rep1,cmt:cmt1,rang:1,cycle:'paie',objet:'F'},
							success:function(e){
							//	alert(e);
							}				
						});
	...
	<body>		
		<div id="GranTitre">
			G - RATTACHEMENT DES OPERATIONS : Partie I	
		</div>		
		<div id="ChampGauche">
			
				<p><strong>Travaux à faire :</strong> Contrôles les charges à payer et les provisions liées au personnel (gratifications, commissions, etc). </p>
				
				<h4>Documents et infos à obtenir</h4>	
				<p>
					<ul>
						<li>Grand livres des comptes.</li>
						<li>Etat récapitulatif des commissions et gratifications de l'exercice.</li>
					</ul>
				</p>
				<h4>Question :</h4>	
				<p>
					<ul>
						<li>Assurez-vous que toutes les commissions et gratifications de l'exercice ont été comptabilisées ?</li>
					</ul>
				</p>
				...
				<div id="uploadJustification" style="display:none;">
					...
					<?php 
						$sql = "SELECT nom,extension FROM tab_pieces_justificatives WHERE mission_id=".$_SESSION['idMission']." AND pour = 'paie_personnel_f1'";
						$rep =$bdd->query($sql);
						
						if($donnee = $rep->fetch()) {
					?>
						<a href="<?php echo "pieces_justificatives/pj_paie_personnel_f1_".$_SESSION['idMission'].'.'.$donnee['extension'] ?>" target="_blank"><?php echo $donnee['nom'] ?></a>
					<?php
						}
					?>
					<form enctype="multipart/form-data" action="RDC/envoi_pieces_justificatives.php" method="POST" id="form" target="uploadFrame">
						...
						<input type="hidden" name="pour" value="paie_personnel_f1" />
		...
		<div>
			...
			<div id="ChampDroite" 
				...
				?> >
				<h5><u>Question :</u></h5>
				<p>Assurez-vous que toutes les commissions et gratifications de l'exercice ont été comptabilisées ?
...
<html>
	...
	<body>		
		<div id="GranTitre">
			F - RATTACHEMENT DES OPERATIONS : Partie I	
		</div>		
		<div id="ChampGauche">
