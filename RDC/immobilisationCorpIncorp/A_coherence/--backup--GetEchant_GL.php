...
$ListEchan_tab=$_POST['listId'];
	...
	foreach ($ListEchan_tab as $GLGEN_ID)
	{
		...
		/*SELECT * 
				FROM tab_gl_gen AS tab1
				INNER JOIN tab_ehantillon_gl AS tab2 
				WHERE tab1.GL_GEN_ID=tab2.id_gl_associe
				AND tab1.GL_GEN_ID=".$ListEchan_tab[$i];*/
		...
		while ($donnees = $reponse->fetch())
		{
				...
				$sold=$donnees['GL_GEN_SOLDE'];
					...
					// var_dump($ListEchan_tab);
					// var_dump($id);
					// var_dump(array_search($id,$ListEchan_tab));
					// $sqlRemove="DELETE FROM tab_ehantillon_gl WHERE id_echantillon_GL>0
					// ";
