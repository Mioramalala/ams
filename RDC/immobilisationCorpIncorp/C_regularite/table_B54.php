...
///////////////////////////compt accessoires/////////////
	$reponse=$bdd->query("select count(tab_Frais_Accessoire.id_frais_acc) As nbr_Acc from tab_Frais_Accessoire,tbl_detaillefraix_acc where  tab_Frais_Accessoire.id_frais_acc=tbl_detaillefraix_acc.id_frais_acc AND  tab_Frais_Accessoire.id_mission=".$mission_id);
		$donnees=$reponse->fetch();
		$nbr_Acc=(int)$donnees['nbr_Acc'];
		// echo $nbr_Acc;
///////////////////////////////////////////////////////
	$reponse=$bdd->query("select nom_frais_acc from tab_Frais_Accessoire,tbl_detaillefraix_acc where
	tab_Frais_Accessoire.id_frais_acc=tbl_detaillefraix_acc.id_frais_acc and  tab_Frais_Accessoire.id_mission=".$mission_id);
...
<table id="tb" style="border-collapse:collapse;">
	...
	<tbody>
			...
			<?php 
				...
				$sql="select id_echantillon_GL,compt_ech_gl,date_ech_gl,journal_ech_gl,ref_ech_gl,libelle_ech_gl,debit_ech_gl,crd_ech_gl,sold_ech_gl,renvoi
						
				from tab_ehantillon_gl_fraixacc 
				where id_mission='$mission_id' and GL_GEN_CYCLE='B- Immobilisations incorporelles et corporelles'" ;
				...
				{ 
					...
					<td><input readonly="readonly" name="prix_achat[]" value="<?php print (number_format((double)$don["prix_achat"], 2, '.', ' ')); ?>" class="prix_" id="prix"></td>
					<?php 
						...
						for ($j=0;$j<$nbr_Acc;$j++)
						 {
								...
								$don_echant=$rps_echant->fetch();
									...
									?>
									<td><input readonly="readonly" name="Fraixacc[]" value="<?php print(number_format((double)$don_echant["prix_fraix"], 2, '.', ' '));   ?>"  class="Fraixacc" ></td>
						...
						?>
							...
							<td><input readonly="readonly" name="cout[]" class="cout"  value="<?php print (number_format((double)$don["cout"], 2, '.', ' ')); ?>"  id="cout"></td>
							<td><input readonly="readonly" name="montant[]"  class="montant" value="<?php print (number_format((double)$donnees["sold_ech_gl"], 2, '.', ' ')); ?>"></td>
							<td><input readonly="readonly"  name="ecart_[]" class="ecart" value="<?php print (number_format((double)$don["ec"], 2, '.', ' ')); ?>" ></td>
							<td><input readonly="readonly" name="observation[]" value="<?php print ($don["observation"]); ?>" id="observation"></td>
							</tr>
