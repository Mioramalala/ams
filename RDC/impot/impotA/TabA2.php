<?php
	...
	$reqSyn=$bdd->query("SELECT * FROM tab_synthese_ra WHERE SYNTHESE_OBJECTIF = 'impot_taxe' AND MISSION_ID='".$_SESSION['idMission']."'");
	$syn = $reqSyn->fetch();
	$reqCon=$bdd->query("SELECT * FROM tab_conclusion_ra WHERE CONCLUSION_OBJECTIF = 'impot_taxe' AND MISSION_ID='".$_SESSION['idMission']."'");
	$con = $reqCon->fetch();
?>
<head>
		<meta charset="utf-8"/>
