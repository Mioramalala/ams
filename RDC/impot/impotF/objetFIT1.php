<?php @session_start();
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$sql1="SELECT RDC_REPONSE,RDC_COMMENTAIRE FROM tab_rdc WHERE RDC_RANG= 1 AND RDC_CYCLE='impot_taxe' AND RDC_OBJECTIF='F' AND MISSION_ID=".$_SESSION['idMission'];
	$rep1=$bdd->query($sql1);
	$don1=$rep1->fetch();
...
<html>
		...
		<script>
			$(function(){
				$("#renvoi").click(function(){
					// $("#ChampGauche").load("RDC/impot/impotF/TabF1.php");
					$('#ChampDroite').show();
					$('#suivantIT').show();
					$('#precedIT').show();
					$('#retourIT').hide();
				...
				$("#suivantIT").click(function(){
					...
					if((rep1=="non" && cmt1=="") || rep1==""){ 
						...
						$.ajax({
							...
							data:{rep:rep1,cmt:cmt1,rang:1,cycle:'impot_taxe',objet:'F'},
							success:function(e){
							//	alert(e);
							}				
						});
	...
	<body>
		...
		<div id="ChampGauche">
				...
				<p><strong>Travaux à faire :</strong> Contrôler les charges à payer, provisions et produits à recevoir liés aux impôts et taxes. </p>
				
				<h4>Documents et infos à obtenir</h4>	
				<p>
					<ul>
						<li>GL des charges à payer, provisions et produits à recevoir liés aux impôts et taxes.</li>
						<li>PJ correspondantes.</li>
					</ul>
				</p>
				<h4>Question :</h4>	
				<p>
					<ul>
						<li>Les opérations enregistrées sont-elles comptabilisées à l'exercice auquel elles se rattachent ?</li>
					</ul>
				</p>
		...
		<div>
			...
			<div id="ChampDroite" 
				...
				?> >
				<h5><u>Question :</u></h5>
				<p>Les opérations enregistrées sont-elles comptabilisées à l'exercice auquel elles se rattachent ?
