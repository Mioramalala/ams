...
include '../../connexion.php';
	...
	$reponse0=$bdd->query('select SYNTHESE_ID_RDC from tab_validation_synthese_rdc where MISSION_ID='.$mission_id.' AND SYNTHESE_CYCLE_RDC="impot"');
	
	$donnees0=$reponse0->fetch();
	
	if($donnees0['SYNTHESE_ID_RDC']!=0){
		$reponse1=$bdd->exec('update tab_validation_synthese_rdc set UTIL_ID = "'.$UTIL_ID.'",	SYNTHESE_RDC="'.$synthese_impot.'" where MISSION_ID="'.$mission_id.'" AND SYNTHESE_ID_RDC='.$donnees0['SYNTHESE_ID_RDC']);
		if(!$reponse1) echo  'update tab_validation_synthese_rdc set UTIL_ID = "'.$UTIL_ID.'",SYNTHESE_RDC="'.$synthese_impot.'" where MISSION_ID="'.$mission_id.'" AND SYNTHESE_ID_RDC='.$donnees0['SYNTHESE_ID_RDC'];
	}
	else{
