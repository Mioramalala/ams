...
<script>
		...
		$('#bt_suivant').click(function(){
			...
			if((reponse1=="non" && commentaire1=="") || reponse1==""){ 
				...
				for(var i=1; i <(lignes+1); i++){
					...
					if(nature!='' || annexe!=''){
						add_Input(nature, annexe, <?php echo $mission_id; ?>);
					}
				}
	...
	function add_Data(reponse,commentaire,cycle,objectif,rang,mission_id){
		...
		function add_Input(nature, note ,mission_id){
		$.ajax({
			type:'POST',
	...
	function delete_table(mission_id){
		$.ajax({
			type:'POST',
			url:'RDC/DCD/G_information/delete_table.php',
