		...
		<td width="100px">OBSERVATIONS</td>		
			...
			?>
					<tr bgcolor="white">
						...
						<?php
							$rep = $bdd->query("SELECT DCD_CIRCULARISATION FROM tab_just_apur_dcd WHERE DCD_OBJECTIF='B' AND 
							DCD_MISSION_ID=".$mission_id." AND DCD_RANG=".$ligne);
							$data = $rep->fetch();
						?>
						<td width="100px"><input type="text" id="<?php echo "circularisation_".$ligne ?>"  value="<?php echo $data['DCD_CIRCULARISATION']; ?>"/></td>
						<?php
							$rep = $bdd->query("SELECT DCD_APUREMENT_N1 FROM tab_just_apur_dcd WHERE DCD_OBJECTIF='B' AND 
							DCD_MISSION_ID=".$mission_id." AND DCD_RANG=".$ligne);
							$data = $rep->fetch();
						?>
						<td width="100px"><input type="text" id="<?php echo "apurement_".$ligne ?>" value="<?php echo $data['DCD_APUREMENT_N1']; ?>"/></td>
						<?php
							$rep = $bdd->query("SELECT DCD_OBSERVATION FROM tab_just_apur_dcd WHERE DCD_OBJECTIF='B' AND 
							DCD_MISSION_ID=".$mission_id." AND DCD_RANG=".$ligne);
							$data = $rep->fetch();
						?>
