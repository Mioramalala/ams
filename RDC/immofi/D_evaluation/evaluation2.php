...
$reponse1=$bdd->query('select RDC_COMMENTAIRE, RDC_REPONSE from tab_rdc where RDC_CYCLE="immofi" and RDC_OBJECTIF="D" and MISSION_ID='.$mission_id.' and RDC_RANG=4');
...
<script>
	...
	$(function(){
		...
		$('#bt_suivant').click(function(){
			...
			if((reponse1=="non" && commentaire1=="") || reponse1==""){ 
				...
				add_Data(reponse1,commentaire1,'immofi','D',4,<?php echo $mission_id; ?>);
	...
	function enregistrerSynthese(){
		...
		$.ajax({
			type:'POST',
			url:'RDC/immofi/D_evaluation/addSyntheseRa.php',
...
<table width="100%" height="470">
	<tr>
		...
		<div id="contenue_travail" style="height:450px;overflow:auto;width:900px;">
			...
			<br/>Contrôler la prise en compte des risques ou dépréciations éventuels sur les titres et créances rattachées aux participations ( examen notamment de la situation des filiales)</label><br/><br/><br/>
			
			<label><strong>Documents et infos à obtenir</strong></label><br />
				<label><strong>.</strong>Tableau de suivi des provisions pour dépréciation des titres et créances rattachées aux participations</label><br/><br/>
				<label><strong>.</strong>Documents pouvant justifier la situation des filiales</label><br/><br/><br/>
				
			<label><strong>Question :</strong></label><br />
				<label>Appréciez-vous le niveau des provisions pour dépréciation comptabilisées ?</label><br/><br/><br/>
			...
			<div id="upload" style="display:none;">
				<div id="uploadJustification">
					...
					<?php 
						$sql = "SELECT nom,extension FROM tab_pieces_justificatives WHERE mission_id=".$_SESSION['idMission']." AND pour = 'immofi_D2'";
						$rep =$bdd->query($sql);
						
						if($donnee = $rep->fetch()) {
					?>
						<a href="<?php echo "pieces_justificatives/pj_immofi_D2_".$_SESSION['idMission'].'.'.$donnee['extension'] ?>" target="_blank"><?php echo $donnee['nom'] ?></a>
					<?php
						}
					?>
					<form enctype="multipart/form-data" action="RDC/envoi_pieces_justificatives.php" method="POST" id="form" target="uploadFrame">
						...
						<input type="hidden" name="pour" value="immofi_D2" />
		...
		<div id="contenue_question" style="overflow;height:450px;">
			...
			?> >
				<label><strong>Question :</strong></label><br />
				<label>Appréciez-vous le niveau des provisions pour dépréciation comptabilisées ?</label>
