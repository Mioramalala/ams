...
if($rdc_id==0){
	$rep=$bdd->exec('insert into tab_rdc(RDC_CYCLE, RDC_COMMENTAIRE, RDC_REPONSE, RDC_OBJECTIF, RDC_RANG, MISSION_ID,UTIL_ID) value ("'.$cycle.'","'.$commentaire.'","'.$reponse.'","'.$objectif.'",'.$rang.','.$mission_id.','.$UTIL_ID.')');
	...
	$rep1=$bdd->exec('update tab_rdc set UTIL_ID = "'.$UTIL_ID.'",RDC_COMMENTAIRE="'.$commentaire.'", RDC_REPONSE="'.$reponse.'" where MISSION_ID="'.$mission_id.'" AND RDC_ID='.$rdc_id);
