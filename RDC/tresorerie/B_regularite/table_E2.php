...
<?php
	$query = 'select cpt from tab_rdc_tr_e2 where mission_id='.$mission_id.' and reference="E2"';
	...
	function enregistreTableE2(donnee){
		if(donnee.avis != '' || donnee.date != '' || donnee.nature != '' || donnee.compta != '' || donnee.observation != '') {
			$.ajax({
				...
				data:{avis : donnee.avis, date : donnee.date, nature : donnee.nature, compta : donnee.compta, observation : donnee.observation, mission_id:<?php echo $mission_id;?>, cpt : donnee.cpt, reference : 'E2'},
				success:function(){
