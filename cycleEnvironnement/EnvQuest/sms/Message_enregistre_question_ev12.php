...
$(function(){
	...
	$('#enregistrer_Fermeture_Question_ev12').click(function(){	
	if((document.getElementById("commentaire_Question_ev12").value=="" && document.getElementById("rad_Question_Non_ev_12").checked== true) || (document.getElementById("rad_Question_Oui_ev_12").checked==false && document.getElementById("rad_Question_Non_ev_12").checked==false)){
			$('#mess_quest_vide_ev12').show();
			$('#Question_ev_416').hide();
			$('#message_fermeture_question_ev12').hide();
		}
		else{
				...
				success:function(e){
					...
					if(objectif_id==0){					
						...
						$.ajax({
							...
							data:{commentaire:commentaire, qcm:qcm, question_id:416},
							success:function(){
								$.ajax({
									type:'POST',
	...
	$('#valider_Fermeture_Question_ev12').click(function(){
		$('#message_fermeture_question_ev12').hide();
		$('#fancybox_ev').hide();
		openButtObjev();
	});
...
<table width="500">
	...
	<tr align="center" height="50">
		<td>
			<input type="button" class="bouton" value="Oui" id="enregistrer_Fermeture_Question_ev12" />&nbsp;&nbsp;&nbsp;
			<input type="button" class="bouton" value="Non" id="valider_Fermeture_Question_ev12" />
		</td>
	</tr>
</table>
</div>