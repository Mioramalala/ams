...
$reponse=$bdd->query('SELECT QUESTION_LIBELLE, OBJECTIF_COMMENTAIRE, OBJECTIF_QCM FROM tab_objectif INNER JOIN tab_question ON tab_objectif.QUESTION_ID=tab_question.QUESTION_ID WHERE QUESTION_CYCLE_ACHAT=31 AND MISSION_ID='.$mission_id);
$compte=0;
$couleur='#efefef';
...
?>	<tr bgcolor="<?php if($couleur=='#efefef'){$couleur='#fff'; echo '#fff';} else {$couleur='#efefef';echo '#efefef';}?>" class="sous_Titre" value="<?php //echo $donnees['MISSION_QUESTION_B_ID']; ?>" onclick="selectionMissionQuestionIdf(this.id,<?php //echo $donnees['MISSION_QUESTION_B_ID']; ?>)" style="cursor:pointer;">
		<td width="360" align="left" height="40"><?php echo utf8_encode($donnees['QUESTION_LIBELLE']); ?></td>
		<td width="50" align="center"><?php echo $donnees['OBJECTIF_QCM']; ?></td>
		<td width="340" align="center"><?php echo utf8_encode($donnees['OBJECTIF_COMMENTAIRE']); ?></td>
	</tr>
<?php
$compte++;
}
