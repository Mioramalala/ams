<?php
	function getAuditeurs($mission_id, $cycle){
		...
		$sql = "SELECT DISTINCT tu.UTIL_NOM FROM tab_distribution_tache td
		INNER JOIN tab_tache t
		ON td.tache_id=t.id_tache
		INNER JOIN tab_utilisateur tu
		ON td.UTIL_ID=tu.UTIL_ID
		WHERE t.processus_tache='RSCI'
		AND t.cycle_tache='".$cycle."'
		AND td.STATUT='actif'
		AND td.TACHE_SUPERVISEUR='non'";
