...
function select_tabvd(id, cpt, mission_id, cptFrm){
	quest_id="#Question_vd_"+cpt;
	$.ajax({
		type:'POST',
		url:'cycleVente/Vented/php/getRepvd.php',
		data:{cpt:cpt, mission_id:mission_id},
		success:function(e){
			doc=e.split(',');
			idcomment="#commentaire_Question_vd"+cptFrm;
			idrisqueoui="rad_Question_Oui_vd_"+cptFrm;
			idrisquena="rad_Question_NA_vd_"+cptFrm;
			idrisquenon="rad_Question_Non_vd_"+cptFrm;
			risque=doc[1];
			if(risque=="OUI"){
			...
			document.getElementById("int_vd_Retour").disabled=true;
			document.getElementById("Int_vd_Continuer").disabled=true;
			document.getElementById("Int_vd_Synthese").disabled=true;
			$(idcomment).val(doc[0]);
			$(quest_id).show();			
		}
	});
...
$reponse=$bdd->query('SELECT QUESTION_LIBELLE, OBJECTIF_QCM, OBJECTIF_COMMENTAIRE, OBJECTIF_ID FROM tab_objectif INNER JOIN tab_question ON tab_objectif.QUESTION_ID=tab_question.QUESTION_ID WHERE MISSION_ID='.$mission_id.' AND CYCLE_ACHAT_ID=28');
...
?>
<tr bgcolor="<?php echo $couleur;?>" <?php if ($couleur=="#fff") echo $couleur="#efefef"; else $couleur="#fff"; ?> onclick="select_tabvd(this.id,<?php echo $cpt; ?>,<?php echo $mission_id;?>,<?php echo $cptFrm;?>)" id="tabc<?php echo $cpt; ?>" height="50">
