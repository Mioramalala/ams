...
$reponse=$bdd->query('SELECT QUESTION_LIBELLE, OBJECTIF_COMMENTAIRE, OBJECTIF_QCM FROM tab_objectif INNER JOIN tab_question ON tab_objectif.QUESTION_ID=tab_question.QUESTION_ID WHERE QUESTION_CYCLE_ACHAT=4 AND MISSION_ID='.$mission_id);
$compte=0;
$couleur='#efefef';
while ($donnees = $reponse->fetch())
{
?>	<tr bgcolor="<?php if($couleur=='#efefef'){$couleur='#fff'; echo '#fff';} else {$couleur='#efefef';echo '#efefef';}?>" class="sous_Titre" value="<?php //echo $donnees['MISSION_QUESTION_B_ID']; ?>" onclick="selectionMissionQuestionIdd(this.id,<?php //echo $donnees['MISSION_QUESTION_B_ID']; ?>)" style="cursor:pointer;">
