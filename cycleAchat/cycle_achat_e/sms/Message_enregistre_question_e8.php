...
$(function(){
	...
	$('#enregistrer_Fermeture_Question_e8').click(function(){	
	if((document.getElementById("commentaire_Question_e8").value=="") || (document.getElementById("rad_Question_Oui_e_8").checked==false && document.getElementById("rad_Question_NA_e_8").checked==false && document.getElementById("rad_Question_Non_e_8").checked==false)){
			...
			$('#Question_e_53').hide();
			$('#message_fermeture_question_e8').hide();
		}
		else{
				...
				success:function(e){
					...
					if(objectif_id==0){					
						...
						$.ajax({
							...
							data:{commentaire:commentaire, qcm:qcm, question_id:53},
							success:function(){
								$.ajax({
									type:'POST',
	...
	$('#valider_Fermeture_Question_e8').click(function(){
		$('#message_fermeture_question_e8').hide();
		$('#fancybox_e').hide();
		openButtObje();
	});
...
<table width="500">
	...
	<tr align="center" height="50">
		<td>
			<input type="button" class="bouton" value="Oui" id="enregistrer_Fermeture_Question_e8" />&nbsp;&nbsp;&nbsp;
			<input type="button" class="bouton" value="Non" id="valider_Fermeture_Question_e8" />
		</td>
	</tr>
</table>
</div>