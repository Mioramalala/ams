...
function compte_cpt8(id, poste_id, cpt, mission_id){
	...
	if(document.getElementById(id_8).checked==true){
		...
		$.ajax({
			...
			data:{poste_id:poste_id, ligne:8},
			success:function(){
			}
		});		
	}
...
?>	
		<td class="titre" width="100" align="center">
			<input type="checkbox" onchange="compte_cpt8(this.id,<?php echo $donnees ['POSTE_CYCLE_ID']; ?>,<?php echo $cpt_col; ?>, <?php echo $mission_id; ?>)" id="id_8<?php echo $cpt_col; ?>" <?php if($donnees1['FONCT_ACHAT_A_RESULT']==1) echo 'checked';?>/>
			<?php
				$cpt_col++;
			?>
		</td>
