...
if (is_uploaded_file($_FILES['file_ImportN1']['tmp_name']))
{ 
	$NameFileko=$_FILES['file_ImportN1']['name'];
?>	

<?php	
	$loader=$_SERVER['DOCUMENT_ROOT']."/RA/archive/".$NameFileko;
	$sourceDoc="/RA/archive/".$NameFileko;
	$result= move_uploaded_file($_FILES['file_ImportN1']['tmp_name'],$loader);
	if ($result != 1)
   { 
			...
			<option value = "N-1">N-1
				<?php
					include '../connexion.php';
						$reponse2=$bdd->query('SELECT IMPORTN1_CHOIX_ANNEE FROM tab_importern1 WHERE IMPORTN1_CHOIX_ANNEE="N-1" AND MISSION_ID='.$mission_id);
							$res2=$reponse2->fetch();
								$data2=$res2['IMPORTN1_CHOIX_ANNEE'];
								if($data2!=" " && $data2=="N-1"){ 
									echo("<script>
								
									var annee2= $('#annee').val();
										if(annee2='N-1'){
											alert('Attention fichier balance générale existe déjà,supprimer ancien et enregistrer nouveau!');
										}
