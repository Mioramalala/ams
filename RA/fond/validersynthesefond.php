...
include '../../connexion.php';
	...
	if($donnees0['SYNTHESE_ID_RA']!=0){
		$reponse1=$bdd->exec('update tab_synthese_ra set UTIL_ID = "'.$UTIL_ID.'",SYNTHESE="'.$synthese_fond.'" where
		MISSION_ID="'.$mission_id.'" AND SYNTHESE_ID_RA='.$donnees0['SYNTHESE_ID_RA']);
		if(!$reponse1) echo  'update tab_synthese_ra set UTIL_ID = "'.$UTIL_ID.'",SYNTHESE="'.$synthese_fond.'" where
		MISSION_ID="'.$mission_id.'" AND SYNTHESE_ID_RA='.$donnees0['SYNTHESE_ID_RA'];
	}
	else{
	...
	$req2='update tab_synthese_ra set UTIL_ID = "'.$UTIL_ID.'",SYNTHESE="'.$synthese_fond.'" where
		MISSION_ID="'.$mission_id.'" AND SYNTHESE_ID_RA='.$donnees0['SYNTHESE_ID_RA'];
	$file = '../../fichier/save_mission/mission.sql';
