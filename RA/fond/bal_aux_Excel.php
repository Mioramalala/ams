...
if (is_uploaded_file($_FILES['file_bal_aux']['tmp_name']))
{ 
	$NameFileko=$_FILES['file_bal_aux']['name'];
	//echo $NameFileko;
?>	
...
<?php	
	...
	$result= move_uploaded_file($_FILES['file_bal_aux']['tmp_name'],$loader);
	if ($result != 1)
   { 
	...
	<html>
					...
					// alert(mission_id);
						$.ajax({
							...
							success:function(e){
								...
								$.ajax({
									type:'POST',
									url:'enregistrement_bal_aux.php',
									data:{cpt1:cpt1,cpt2:cpt2,cpt3:cpt3,cpt4:cpt4,cpt5:cpt5,cpt6:cpt6,cpt7:cpt7,mission_id:mission_id,entrepriseId:entrepriseId,annee:annee,fileName:fileName},
									success:function(e1){
		...
		Exercice:<select id = "annee" onchange="choix_annee()">
			<option value = "N">N
				<?php
					include '../connexion.php';
						$reponse=$bdd->query('SELECT BAL_AUX_CHOIX_ANNEE FROM tab_importer WHERE BAL_AUX_CHOIX_ANNEE="N" AND MISSION_ID='.$mission_id);
							$res=$reponse->fetch();
								$data1=$res['BAL_AUX_CHOIX_ANNEE'];
								if($data1!=" " && $data1=="N"){ 
									echo("<script>
									var annee= $('#annee').val();
										if(annee='N'){
											alert('Attention fichier balance auxiliaire existe déjà!');
										}
									</script>");
								}
				?>
			...
			<option value = "N-1">N-1
				<?php
					include '../connexion.php';
						$reponse2=$bdd->query('SELECT BAL_AUX_CHOIX_ANNEE FROM tab_importer WHERE BAL_AUX_CHOIX_ANNEE="N-1" AND MISSION_ID='.$mission_id);
							$res2=$reponse2->fetch();
								$data2=$res2['BAL_AUX_CHOIX_ANNEE'];
								if($data2!=" " && $data2=="N-1"){ 
									echo("<script>
