		  ...
		  for($i=0;$i<$nbrres;$i++)
		   {
						...
						if($compte==0)
						{
							$reponse=$bdd->exec('INSERT INTO tab_ra(RA_COMPTE,RA_LIBELLE,RA_D,RA_C,RA_SOLDE_N,RA_SOLDE_N1,RA_VARIATION,RA_POURCENTAGE,RA_COMMENTAIRE,MISSION_ID,	RA_CYCLE,UTIL_ID) VALUES ("'.$enregistrer_tableau_import1[$i].'","'.$enregistrer_tableau_import2[$i].'","'.$enregistrer_tableau_import3[$i].'", "'.$enregistrer_tableau_import4[$i].'","'.$enregistrer_tableau_import5[$i].'","'.$enregistrer_tableau_import6.'", "'.$enregistrer_tableau_import7.'","'.$enregistrer_tableau_import8.'","'.$enregistrer_tableau_import9[$i].'","'.$mission_id.'","paie","'.$UTIL_ID.'")');
