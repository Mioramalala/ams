...
<html>
			...
			function enregistrer_vente(){
				...
				$.get("droitAccUtil.php",function (e)
				{
					...
					{		
						    $.ajax({
							type:'POST',
							url:'RA/planification/vente/enregistrervente.php',
							data:{fonction:fonction,compte:compte,planif_gen:planif_gen,seuil_sign:seuil_sign,taux_sondage:taux_sondage,mission_id:mission_id},
								success:function(e){
								alert('VENTES bien enregistr\351s');	
	...
	<body>
		<table width="100%" height="50" bgcolor="#00698d" style="text-align:left;">
				<tr>
					<td style="color:white"><center>VENTES DES PRODUITS CLIENTS</center></td>
				</tr>
		</table>
		<div id="dv_fond">
				...
				<table class="fond1">
					<tr>
						<td class="td_fond"><p class="titre" style="color:blue;">INCIDENCES</p>
							...
							<?php 
								$reponse=$bdd->query("select FONCTIONNEMENT,CYCLE from tab_incidence_ra where CYCLE='G' AND MISSION_ID=".$mission_id);
					...
					<tr>
						...
						<?php 
							$reponse=$bdd->query("select COMPTE,CYCLE from tab_incidence_ra where CYCLE='G' AND MISSION_ID=".$mission_id);
								while($donnees=$reponse->fetch()){
								$cpt=$donnees['COMPTE'];
							?>
			...
			<div id="dv_fond_planif2">
				<table class="fond2">
					<tr>
						<td class="td_fond"><p class="titre" style="color:blue;">PLANIFICATION VENTES</p>
							<p id="ligne"></p>
							<p class="td_fond2">Planification par cycle :
							<textarea rows="2px" cols="70px" id="planif_gen" placeholder="Rédiger ici votre planification des ventes">
								<?php echo getPlanif("vente", $mission_id); ?>
							</textarea>
							</p>
					</tr>
					<tr>
						...
						<textarea rows="2px" cols="70px" id="seuil_sign" placeholder="Rédiger ici votre seuil de signification">
							<?php echo getSeuil("vente", $mission_id); ?>
						</textarea>
						</p></td>
					</tr>
					<tr>
						...
						<textarea rows="2px" cols="70px" id="taux_sondage" placeholder="Rédiger ici votre taux de sondage">
							<?php echo getSondage("vente", $mission_id); ?>
						</textarea>
						</p></td>
					</tr>
				</table>
		...
		<div id="id_btn">
			<table>
				<tr>
					<td>
						<input class="btn_planif" type="button" value="Enregistrer" onClick="enregistrer_vente();">
						<input class="btn_planif" type="button" value="Retour" onClick="retour_vente();">
					</td>
				</tr>
			</table>
		</div>
