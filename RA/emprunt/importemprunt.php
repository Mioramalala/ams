...
if (is_uploaded_file($_FILES['file_Import_emprunt']['tmp_name']))
{ 
	$NameFileko=$_SESSION['nom_entreprise'].$mission_id.$_FILES['file_Import_emprunt']['name'];
	echo $NameFileko;
?>	
	
<?php	
	$loader=$_SERVER['DOCUMENT_ROOT']."/RA/emprunt/uploads/".$NameFileko;
	$sourceDoc="/RA/emprunt/uploads/".$NameFileko;
	$result= move_uploaded_file($_FILES['file_Import_emprunt']['tmp_name'],$loader);
	if ($result != 1)
   { 
