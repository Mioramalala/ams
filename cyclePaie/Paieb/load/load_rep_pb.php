...
function select_tabpb(id, cpt, mission_id, cptFrm){
	quest_id="#Question_pb_"+cpt;
	$.ajax({
		type:'POST',
		url:'cyclePaie/Paieb/php/getReppb.php',
		data:{cpt:cpt, mission_id:mission_id},
		success:function(e){
			doc=e.split(',');
			idcomment="#commentaire_Question_pb"+cptFrm;
			idrisqueoui="rad_Question_Oui_pb_"+cptFrm;
			idrisquena="rad_Question_NA_pb_"+cptFrm;
			idrisquenon="rad_Question_Non_pb_"+cptFrm;
			risque=doc[1];
			if(risque=="OUI"){
			...
			document.getElementById("int_pb_Retour").disabled=true;
			document.getElementById("Int_pb_Continuer").disabled=true;
			document.getElementById("Int_pb_Synthese").disabled=true;
			$(idcomment).val(doc[0]);
			$(quest_id).show();			
		}
	});
...
$reponse=$bdd->query('SELECT QUESTION_LIBELLE, OBJECTIF_QCM, OBJECTIF_COMMENTAIRE, OBJECTIF_ID FROM tab_objectif INNER JOIN tab_question ON tab_objectif.QUESTION_ID=tab_question.QUESTION_ID WHERE MISSION_ID="'.$mission_id.'" AND CYCLE_ACHAT_ID=14');
...
?>
<tr bgcolor="<?php echo $couleur;?>" <?php if ($couleur=="#fff") echo $couleur="#efefef"; else $couleur="#fff"; ?> onclick="select_tabpb(this.id,<?php echo $cpt; ?>,<?php echo $mission_id;?>,<?php echo $cptFrm;?>)" id="tabc<?php echo $cpt; ?>" height="50">
