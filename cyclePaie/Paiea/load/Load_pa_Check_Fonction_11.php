...
<script>
function compte_cptpa11(id, poste_id, cpt, mission_id){
	id_11="id_11"+cpt;
	if(document.getElementById(id_11).checked==true){
		$.ajax({
			type:'POST',
			url:'cyclePaie/Paiea/php/add_pa.php',
			data:{poste_id:poste_id, mission_id:mission_id, ligne:11, result:1, cycleName:'paie'},
			success:function(){
			}
		});
	}
		...
		$.ajax({
			...
			data:{poste_id:poste_id, ligne:11, cycleName:'paie', mission_id:mission_id},
			success:function(){
			}
		});		
	}
...
?>	
		<td class="titre" width="100" align="center">
			<input type="checkbox" onchange="compte_cptpa11(this.id,<?php echo $donnees ['POSTE_CYCLE_ID']; ?>,<?php echo $cpt_col; ?>, <?php echo $mission_id; ?>)" id="id_11<?php echo $cpt_col; ?>" <?php if($donnees1['FONCT_A_RESULT']==1) echo 'checked';?> <?php if($conclusionId!=0) echo 'disabled'; ?> />
			<?php
				$cpt_col++;
			?>
		</td>
