...
//check ligne : verifie si chaqhe ligne de la separation de fonction est checkée, pour paiea il ya 16 lignes à cocher
$sql = "SELECT COUNT(DISTINCT `FONCT_A_LIGNE`) AS nombreLigne
			...
			AND tc.`POSTE_CYCLE_NOM` = 'paie'
			AND tc.`MISSION_ID` = ".$missionId."
			AND tc.`ENTREPRISE_ID` = ".$entrepriseId."
			AND tf.`FONCT_A_NOM`= 'paie'";
		
$req = $bdd->query($sql);
$rep = $req->fetch();
if($rep["nombreLigne"] >= 16) {
	$checkLigne = true;
	$result = 1;
}
