<?php 
	...
	switch ($cycle) {
		case 'immofi':
			...
			$and = " AND rubrique_cycle NOT IN('immoinco','immoco')";
			break;
		case 'immoinco':
			$cycle = "immo";
			$titre = "Immobilisations incorporelles";
			$and = " AND rubrique_cycle NOT IN('immofi','immoco')";
			break;
		case 'immoco':
			$cycle = "immo";
			$titre = "Immobilisations corporelles";
			$and = " AND rubrique_cycle NOT IN('immofi','immoinco')";
			break;
				...
				<table class="table table-hover">
					...
					<?php 
						$sql = "select RA_ID, RA_COMPTE, RA_LIBELLE,RA_SOLDE_N, RA_SOLDE_N1, MISSION_ID, RA_CYCLE,  rubrique_id,sous_rubrique, Type
								from tab_ra
								WHERE MISSION_ID = ".$mission_id ." AND RA_CYCLE like '".$cycle."' ".$and."  ORDER BY RA_COMPTE ASC";
					...
					<tr id="<?php echo "ligne".$idra; ?>">
						...
						<select name="typeBA" id="<?php echo "typeBA".$idra; ?>">
							<option value="">&nbsp;</option>
							<option value="A" <?php if($ra['Type']=="A"){$oui=1;echo "selected";} ?>>Amortissement</option>
							<option value="B" <?php if($ra['Type']=="B"){$oui=1;echo "selected";} ?>>Brut</option>
							</select>
							</td>
						<td>
						<?php if($oui==0): ?>
...
// 	//suivant attribution rubrique
	$("#generer").click(function(){
		// if(rowCount > 0){
			$("#contenue_RA").load("Rap_Inter/notesAnnexes/gen_tableau/recap_N_N1.php?p="+cycle);
		// } else {
