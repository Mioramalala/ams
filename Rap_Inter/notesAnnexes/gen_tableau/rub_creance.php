<?php 
	...
	$titre = "Créance et emplois assimilés";
				...
				<table class="table table-hover">
					...
					<?php 
						$sql = "select RA_ID, RA_COMPTE, RA_LIBELLE,RA_SOLDE_N, RA_SOLDE_N1, MISSION_ID, RA_CYCLE,  rubrique_id, sous_rubrique, Type
								from tab_ra
								WHERE MISSION_ID = ".$mission_id ." AND RA_CYCLE IN (".$cycle.") AND rubrique_cycle NOT like 'passifco' ORDER BY RA_COMPTE ASC";
					...
					<tr id="<?php echo "ligne".$idra; ?>">
						...
						<td>
							...
							<option value="">&nbsp;</option>
							<?php 
								$sql = "SELECT `rubrique_id`, `rubrique_libelle`, `rubrique_cycle` FROM `tab_rubrique_notes_annexes` WHERE `rubrique_cycle` like 'charge' ORDER BY `rubrique_cycle` ASC ";
						...
						<td>
							<select name="sous_rubrique" id="<?php echo "sous_rubrique".$idra; ?>" class="select_sous_rubrique" title="Sous rubrique">
							<option value="">&nbsp;</option>
							<?php
...
// 	//suivant attribution rubrique
	$("#generer").click(function(){
		if(rowCount > 0){
			$("#contenue_RA").load("Rap_Inter/notesAnnexes/gen_tableau/recap_N_N1creance.php?p="+cycle);
		} else {
